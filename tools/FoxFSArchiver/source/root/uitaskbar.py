...
class ExpandedTaskBar(ui.ScriptWindow):
	...
	def LoadWindow(self):
		try:
			...
			dbg.TraceError("ExpandedTaskBar.LoadWindow.Load - %s : %s"%(type,msg))
			app.Abort()
			return 0
		...
		#if app.ENABLE_DUNGEON_INFO_SYSTEM:
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_DUNGEONINFO] = self.GetChild("dungeoninfo_button")
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_DUNGEONINFO].SetParent(self)
		#
		#if app.ENABLE_BIOLOGIST_UI:
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_BIOLOGIST] = self.GetChild("biologist_button")
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_BIOLOGIST].SetParent(self)
		#
		if app.USE_BATTLEPASS:
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_BATTLEPASS] = self.GetChild("battlepass_button")
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_BATTLEPASS].SetParent(self)

		if app.ENABLE_REWARD_SYSTEM:
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_REWARD] = self.GetChild("reward_button")
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_REWARD].SetParent(self)

		#
		#if app.ENABLE_SWITCHBOT_WORLDARD:
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_SWITCHBOT] = self.GetChild("switchbot_button")
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_SWITCHBOT].SetParent(self)
		#
		#if app.ENABLE_TRACK_WINDOW:
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_WORLDBOSS] = self.GetChild("worldboss_button")
		#	self.toggleButtonDict[ExpandedTaskBar.BUTTON_WORLDBOSS].SetParent(self)

		if app.USE_AUTO_HUNT:
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_HUNTING] = self.GetChild("autohunt_button")
			self.toggleButtonDict[ExpandedTaskBar.BUTTON_HUNTING].SetParent(self)

		self.RePositionButton()

	def RePositionButton(self):
		total_count = len(self.toggleButtonDict)
		...
		pos_x = 0
		pos_y = 0
		for k, v in self.toggleButtonDict.items():
...
class TaskBar(ui.ScriptWindow):
	...
	def RefreshExpBtn(self):
		if self.mouseRightButtonModeButton:
			upFilePath = "icon/item/inelantiexp.tga"
			overFilePath = "icon/item/inelantiexp_hover.tga"
			downFilePath = "icon/item/inelantiexp_down.tga"
			
			if constinfo.exp_status:
				upFilePath = "icon/item/inelantiexp_off.tga"
				overFilePath = "icon/item/inelantiexp_off_hover.tga"
				downFilePath = "icon/item/inelantiexp_off_down.tga"

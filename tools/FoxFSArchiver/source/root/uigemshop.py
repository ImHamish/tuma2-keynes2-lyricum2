...
class GemShopWindow(ui.ScriptWindow):
	...
	def OverInItem(self, slotIndex):
		...
		if 0 != self.tooltipItem:
			self.tooltipItem.SetItemToolTip(player.GetGemShopItemVnum(slotIndex), isGayaShop = 1)
	...
	def UseItemSlot(self, slotIndex):
		...
		if player.GetGemShopItemStatus(slotIndex) == 1:
			haveCount = player.GetItemCountByVnum(self.GEM_SHOP_ADD_ITEM_VNUM)
	...
	def SendRequestRefresh(self):
		haveCount = player.GetItemCountByVnum(self.GEM_SHOP_REFRESH_ITEM_VNUM)
		if haveCount < 1:

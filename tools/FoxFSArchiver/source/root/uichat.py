...
class ChatLine(ui.EditLine):
	...
	def GetLastShoutTime(self):
		return self.lastShoutTime

	def SetLastShoutTime(self, lastShoutTime):
		self.lastShoutTime = lastShoutTime
...
#				return
	...
	def SendChatPacket(self, text, type):
		self.__SendChatPacket(text, type)
	...
	def __SendShoutChatPacket(self, text):
		...
		constinfo.LAST_CHAT_SHOUT = text[1:]
...
class ChatInputSet(ui.Window):
	...
	def SendAutoShoutChatPacket(self, text):
		if not self.chatLine or not text:
			return

		lastShoutTime = self.chatLine.GetLastShoutTime()
		if lastShoutTime and app.GetTime() < lastShoutTime + 15:
			return

		self.chatLine.SetLastShoutTime(app.GetTime())
		self.chatLine.SendChatPacket(text, chat.CHAT_TYPE_SHOUT)
...
class ChatWindow(ui.Window):
	...
	def __init__(self, interface):
		...
		btnAutoSendShout = ui.Button()
		btnAutoSendShout.SetParent(self)
		btnAutoSendShout.SetUpVisual("chat/auto_shout_norm.png")
		btnAutoSendShout.SetOverVisual("chat/auto_shout_over.png")
		btnAutoSendShout.SetDownVisual("chat/auto_shout_norm.png")
		btnAutoSendShout.SetToolTipText(localeinfo.CHAT_AUTO_SEND_CHAT_ON)
		btnAutoSendShout.SetEvent(ui.__mem_func__(self.ToggleAutoSendShout))
		btnAutoSendShout.Hide()
		self.btnAutoSendShout = btnAutoSendShout
	...
	if app.USE_CAPTCHA_SYSTEM:
		def hide_btnChatSizing(self):
			if self.btnChatSizing and self.IsShow():
				self.btnChatSizing.Hide()

		def show_btnChatSizing(self):
			if self.btnChatSizing and self.btnChatLog.IsShow():
	...
	if app.ENABLE_NEW_CHAT:
		def SaveOption(self, my):
			...
			fileName = "UserData/chatting/" + player.GetName() + ".data"
	...
	def OpenChat(self):
		...
		self.btnAutoSendShout.SetPosition(self.GetWidth() - 105, 2)
		self.btnAutoSendShout.Show()

		if constinfo.USE_AUTO_LAST_CHAT_SHOUT:
			self.btnAutoSendShout.SetUp()
			self.btnAutoSendShout.SetToolTipText(localeinfo.CHAT_AUTO_SEND_CHAT_OFF)
		else:
			self.btnAutoSendShout.Down()
			self.btnAutoSendShout.SetToolTipText(localeinfo.CHAT_AUTO_SEND_CHAT_ON)
	...
	def ToggleAutoSendShout(self):
		if not constinfo.USE_AUTO_LAST_CHAT_SHOUT:
			self.btnAutoSendShout.SetUp()
			self.btnAutoSendShout.SetToolTipText(localeinfo.CHAT_AUTO_SEND_CHAT_OFF)
			constinfo.AUTO_LAST_CHAT_SHOUT = constinfo.LAST_CHAT_SHOUT
			constinfo.USE_AUTO_LAST_CHAT_SHOUT = True
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeinfo.CHAT_AUTO_SEND_CHAT_IS_ON)
		else:
			self.btnAutoSendShout.Down()
			self.btnAutoSendShout.SetToolTipText(localeinfo.CHAT_AUTO_SEND_CHAT_ON)
			constinfo.USE_AUTO_LAST_CHAT_SHOUT = False
			constinfo.AUTO_LAST_CHAT_SHOUT = None
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeinfo.CHAT_AUTO_SEND_CHAT_IS_OFF)

	def OnAutoSendShout(self):
		if not constinfo.USE_AUTO_LAST_CHAT_SHOUT or not self.chatInputSet:
			return

		if not constinfo.AUTO_LAST_CHAT_SHOUT:
			return

		self.chatInputSet.SendAutoShoutChatPacket(constinfo.AUTO_LAST_CHAT_SHOUT)

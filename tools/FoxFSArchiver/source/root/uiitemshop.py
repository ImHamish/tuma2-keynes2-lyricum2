...
itemShopCategoryData = {
	0 : {
			...
			"subCategory" : [ 
								localeinfo.CATEGORY_1_SUB_0_NAME,
								localeinfo.CATEGORY_1_SUB_1_NAME,
								localeinfo.CATEGORY_1_SUB_2_NAME,
								# localeinfo.CATEGORY_1_SUB_3_NAME,
			],
		},
	1 : {
			"name" : localeinfo.CATEGORY_2_NAME,
			"subCategory" : [ 
								localeinfo.CATEGORY_2_SUB_0_NAME,
								localeinfo.CATEGORY_2_SUB_1_NAME,
								localeinfo.CATEGORY_2_SUB_2_NAME,
			],
		},
	2 : {
			"name" : localeinfo.CATEGORY_3_NAME,
			"subCategory" : [ 
								# localeinfo.CATEGORY_3_SUB_0_NAME,
								# localeinfo.CATEGORY_3_SUB_1_NAME,
			],
		},
	3 : {
			"name" : localeinfo.CATEGORY_4_NAME,
			"subCategory" : [ 
								localeinfo.CATEGORY_4_SUB_0_NAME,
								localeinfo.CATEGORY_4_SUB_1_NAME,
								localeinfo.CATEGORY_4_SUB_2_NAME,
								# localeinfo.CATEGORY_4_SUB_3_NAME,
								# localeinfo.CATEGORY_4_SUB_4_NAME,
								# localeinfo.CATEGORY_4_SUB_5_NAME,
			],
		},
	4 : {
			"name" : localeinfo.CATEGORY_5_NAME,
			"subCategory" : [ 
								localeinfo.CATEGORY_5_SUB_0_NAME,
								localeinfo.CATEGORY_5_SUB_1_NAME,
			],
		},
	# 5 : {
			# "name" : localeinfo.CATEGORY_6_NAME,
			# "subCategory" : [ 
								# localeinfo.CATEGORY_6_SUB_0_NAME,
								# localeinfo.CATEGORY_6_SUB_1_NAME,
			# ],
		# },
	# 6 : {
			# "name" : localeinfo.CATEGORY_7_NAME,
			# "subCategory" : [ 
								# localeinfo.CATEGORY_7_SUB_0_NAME,
								# localeinfo.CATEGORY_7_SUB_1_NAME,
								# localeinfo.CATEGORY_7_SUB_2_NAME,
	...
	# 7 : {
			...
			# "subCategory" : [ 
								# localeinfo.CATEGORY_8_SUB_0_NAME,
								# localeinfo.CATEGORY_8_SUB_1_NAME,
								# localeinfo.CATEGORY_8_SUB_2_NAME,
...
# if app.ENABLE_HALLOWEEN_EVENT_2022:
	# itemShopCategoryData[5] = {
								# "name" : localeinfo.CATEGORY_6_NAME,
								# "subCategory" : [ 
													# localeinfo.CATEGORY_6_SUB_0_NAME,
													# localeinfo.CATEGORY_6_SUB_1_NAME,
													# localeinfo.CATEGORY_6_SUB_2_NAME,
													# localeinfo.CATEGORY_6_SUB_3_NAME,
													# localeinfo.CATEGORY_6_SUB_4_NAME,
													# localeinfo.CATEGORY_6_SUB_5_NAME,
													# localeinfo.CATEGORY_6_SUB_6_NAME,
								# ],
	# }
...
class ItemShopWindow(ui.Window):
	...
	def LoadWindow(self):
		...
		if localeinfo.IsARABIC():
			dragonCoinText.SetTextType("horizontal#right")
		else:
			dragonCoinText.SetTextType("horizontal#left")
		dragonCoinText.SetFontName("Tahoma:15")
		...
		if not app.USE_WHEEL_OF_FORTUNE:
			wheelBtn = ui.Button()
			...
			wheelBtn.SetEvent(self.OnPressWheelButton)
			wheelBtn.SetPosition(180+((buyCoinBtn.GetWidth()+7)*3),12)
			wheelBtn.Show()
			...
			wheelBtnText.SetText(localeinfo.WHEEL_BTN_TEXT)
			wheelBtnText.Show()
		...
		itemSearch.SetEscapeEvent(ui.__mem_func__(self.OnPressEscapeKey))
		itemSearch.Show()
		...
		if localeinfo.IsARABIC():
			itemSearchText.SetTextType("horizontal#right")
		else:
		...
		itemSearchText.Show()
		itemSearch.overLay = itemSearchText
		...
		categoryListBox.SetPosition(20,76)
	...
	if not app.USE_WHEEL_OF_FORTUNE:
		def OnPressWheelButton(self):
			interface = newconstinfo.GetInterfaceInstance()
			if interface:
				interface.OpenWheelofFortune()
				if interface.wndWheelofFortune and interface.wndWheelofFortune.IsShow():
					interface.wndWheelofFortune.SetTop()
	...
	def BuyDragonCoin(self):
		self.LoadFirstOpening()
		net.SendChatPacket("/in_game_buycoins")
	...
	def SetDragonCoinReal(self, dragonCoinValue):
		text = NumberToMoneyFormat(dragonCoinValue)+"\n|cffF8BE47"+localeinfo.ITEMSHOP_DRAGONCOINT1+"|cffF8BE47"
		if localeinfo.IsARABIC():
			self.children["dragonCoinText"].SetTextAE(text,10)
		else:
	...
	def Open(self):
		if not self.children.has_key("sendPacketData"):
			...
			self.LoadFirstOpening()
			global UPDATE_TIME
			net.SendChatPacket("/m_is data %d" % UPDATE_TIME)
...
class BuyWindow(ItemShopBoard):
	...
	def __init__(self):
		...
		yesBtn.SetDownVisual(IMG_DIR+"buy_2.tga")
		yesBtn.SetText(localeinfo.GUILDLAND_ACCEPT)
		yesBtn.Show()
	...
	def SetText(self, text):
		...
		buyText.SetPosition(self.GetWidth()/2,28)
		self.children["yesBtn"].SetPosition((self.GetWidth()/2)-self.children["cancelBtn"].GetWidth()-15,94)
		self.children["cancelBtn"].SetPosition((self.GetWidth()/2)+15,94)
...
class ItemShopItem(ui.ExpandedImageBox):
	...
	def SetTime(self, time):
		...
		if time != 0:
			...
			if localeinfo.IsARABIC():
				timeText.SetPosition(148-60,65+40)
			else:
	...
	def AcceptBuy(self):
		net.SendChatPacket("/m_is buy %d %d"%(self.data[LIST_ITEM_ID],self.buyCount))
	...
	def SetPrice(self, price, discount):
		...
		if localeinfo.IsARABIC():
			itemPrice.SetTextType("horizontal#right")
		else:
			itemPrice.SetTextType("horizontal#left")
		itemPrice.SetFontName("Tahoma:14")
		...
		if discount > 0:
			...
			if localeinfo.IsARABIC():
				itemPriceEx.SetHorizontalAlignRight()
			...
			if localeinfo.IsARABIC():
				lineImage.SetPosition(17+35,(ySize/2)+1)
			else:
	...
	def SetItemName(self):
		...
		if localeinfo.IsARABIC():
			itemName.SetWindowHorizontalAlignRight()
			itemName.SetFontName("Tahoma:15")
			itemName.SetPosition(95-20,15)
			name = item.GetItemName()
			if len(name) > 24:
				name = name[0 : 24]
				name += "..."
			
			itemName.SetText("|cffF7BD48 "+ name +"|cffF7BD48 ")
		else:
			...
			itemName.SetText("|cffF7BD48" + name)
	...
	def OverInItem(self, itemVnum):
		...
		if interface:
			if interface.tooltipItem:
				interface.tooltipItem.SetItemToolTip(itemVnum, isItemShop = 1)
...
class MultiTextLine(ui.Window):
	...
	def SetTextRange2(self, range,range2):
		self.rangeText = range
		self.rangeText2 = range2
		yPosition = 0
		xPosition = 0
		for text in self.children:
			text.SetPosition(yPosition,0)
			yPosition+=self.rangeText
		for text in self.children:
			text.SetPosition(0,xPosition)
			xPosition+=self.rangeText2

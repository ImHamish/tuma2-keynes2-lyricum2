...
USE_MOUNT_COSTUME_SYSTEM = 0
ENABLE_ACCE_COSTUME_SYSTEM = 0
ENABLE_WEAPON_COSTUME_SYSTEM = 0
ENABLE_APPLY_MAGICRES_REDUCTION = 0
...
try:
	if app.USE_MOUNT_COSTUME_SYSTEM:
		USE_MOUNT_COSTUME_SYSTEM = 1
except:
	pass
...
try:
	if app.ENABLE_APPLY_MAGICRES_REDUCTION:
		ENABLE_APPLY_MAGICRES_REDUCTION = 1
except:
	pass
...
class NewOfflineShopBoard(ui.ScriptWindow):
	...
	if ENABLE_APPLY_MAGICRES_REDUCTION:
		ATTRIBUTES[item.APPLY_RESIST_MAGIC_REDUCTION] = localeinfo.OFFLINESHOP_ATTR_RESIST_MAGIC_REDUCTION
	...
	def AuctionList_SetItemInfo(self, vnum, count, *args):
		...
		elm['count'] 	= count
		elm['vnum']		= vnum
		if app.ENABLE_CHANGELOOK and len(args) != 0:
			elm["trans"] = args[0]
	...
	def __OnClickCreateShopButton(self):
		import background
		if background.GetCurrentMapName() not in OFFLINESHOP_MAP_ALLOWS:
			self.__PopupMessage(localeinfo.OFFLINESHOP_CANNOT_CREATE_SHOP_HERE)
			return
	...
	def __OnLeftClickShopItem(self, slot):
		if app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_RCONTROL) and not app.DISABLE_OFFSHOP_OFFERS:
	...
	def __OnLeftClickSearchFilterShopResultItem(self, slot):
		if app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_LCONTROL) and not app.DISABLE_OFFSHOP_OFFERS:
	...
	def __SetTooltip(self,info,canRemove=False):
		...
		if app.ATTR_LOCK:
			self.itemTooltip.AddItemData(info["vnum"], sockets, attrs, lockedattr = info.get('locked_attr', -1), transmutation = trans)
		else:
			self.itemTooltip.AddItemData(info["vnum"], sockets, attrs, transmutation = trans)
		...
		if is_building and not info.get('sold', False):
			...
			if not app.DISABLE_OFFSHOP_OFFERS:

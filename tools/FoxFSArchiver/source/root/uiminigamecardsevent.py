...
class OkeyCardsEventGameWaitingPage(ui.ScriptWindow):
	OKEY_CARD_PLAY_PRICE = "50.000.000"
	...
	def LoadWindow(self):
		try:
			...
			import exception
			exception.Abort("OkeyCardsEventGameWaitingPage.LoadDialog.LoadScript")
		
		try:
			self.titleBar = self.GetChild("titlebar")
			self.textBoard = self.GetChild("desc_board")
			self.scrollBar = self.GetChild("scrollbar")
			self.checkButton = self.GetChild("check_image")
			self.checkButton2 = self.GetChild("confirm_check_button")
			self.startButton = self.GetChild("game_start_button")
		except:
			import exception
			exception.Abort("OkeyCardsEventGameWaitingPage.LoadDialog.BindObject")
	...
	def __OnStartQuestion(self):
		...
		questionDialog.SetText1(localeinfo.MINI_GAME_RUMI_START_QUESTION % (self.OKEY_CARD_PLAY_PRICE, 1))
	...
	def __SetDescriptionEvent(self):
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet(uiscriptlocale.CARDS_DESC_FILE_NAME)
...
class MiniGameCardsPage(ui.ScriptWindow):
	CARDS_ICONS = {
	...
	def LoadWindow(self):
		try:
			...
			import exception
			exception.Abort("MiniGameCards.LoadDialog.LoadScript")

		try:
			self.titleBar = self.GetChild("titlebar")
			self.handSlot = self.GetChild("HandCardSlot")
			self.fieldSlot = self.GetChild("FieldCardSlot")
			self.deckSlot = self.GetChild("DeckCardSlot")
			self.score_completion_effect1 = self.GetChild("score_completion_effect1")
			self.score_completion_effect2 = self.GetChild("score_completion_effect2")
			self.score_completion_effect3 = self.GetChild("score_completion_effect3")
			self.score_completion_text_effect = self.GetChild("score_completion_text_effect")
			self.deck_flush_effect = self.GetChild("deck_flush_effect")
			self.cards_count = self.GetChild("card_cnt_text")
			self.total_points = self.GetChild("total_score")
			self.field_points = self.GetChild("score_number_text")
			self.exitButton = self.GetChild("game_exit_button")
		except:
			import exception
			exception.Abort("MiniGameCards.LoadDialog.BindObject")
		...
		self.score_completion_effect1.SetEndFrameEvent(ui.__mem_func__(self.__OnEndFrame))
...
class MiniGameOkeyCards(ui.Window):
	wndWaitingPage = None
	wndGamePage = None

	def __init__(self):
		ui.Window.__init__(self)
		self.SetWindowName("MiniGameOkeyCards")
		self.wndWaitingPage	= OkeyCardsEventGameWaitingPage()
		self.wndGamePage = MiniGameCardsPage()

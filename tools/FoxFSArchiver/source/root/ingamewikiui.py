...
categoryPeakWindowSize = [109, 404]
...
class WikiRenderTarget(ui.WikiRenderTarget):
	...
	def ManageModelViewWindow(self, deleteModule = False):
		if deleteModule:
			wiki.UnegisterModelViewWindow(self.moduleID, self.hWnd)
		else:
	...
	if app.ENABLE_AURA_SYSTEM:
		def SetModelAura(self, aura_vnum):
			if self.IsValidModuleID():
				wiki.SetModelAura(self.moduleID, aura_vnum)
...
class WikiMainCostumeWindow(ui.Window):
	...
	def OnOverIn(self, vnum, metinSlot = [0 for i in xrange(player.METIN_SOCKET_MAX_NUM)]):
		...
		if app.USE_MOUNT_COSTUME_SYSTEM and itemType == item.ITEM_TYPE_COSTUME and subType == item.COSTUME_TYPE_MOUNT:
			for i in xrange(item.ITEM_APPLY_MAX_NUM):
				(affectType, affectValue) = item.GetAffect(i)
				if app.USE_MOUNT_COSTUME_SYSTEM and affectType == item.APPLY_MOUNT:
					self.wikiRenderTarget.SetModel(affectValue)
					break
			...
			if itemType == item.ITEM_TYPE_WEAPON or itemType == item.ITEM_TYPE_COSTUME and subType == item.COSTUME_TYPE_WEAPON:
				...
				self.wikiRenderTarget.SetModelSash(vnum)
				if vnum in stole_costumes_effect:
					self.wikiRenderTarget.SetModelArmorEffect(stole_costumes_effect[vnum])
			elif app.ENABLE_AURA_SYSTEM and itemType == item.ITEM_TYPE_COSTUME and subType == item.COSTUME_TYPE_AURA:
				self.wikiRenderTarget.SetModelAura(vnum)
...
def BuildBaseMain(self):
	...
	self.categ.SetPosition(13, self.searchBG.GetLocalPosition()[1] + self.searchBG.GetHeight() + 25)
	self.categ.Show()

	self.scrollBar = WikiScrollBar()
	self.scrollBar.SetParent(self.baseBoard)
	self.scrollBar.SetPosition(self.categ.GetLocalPosition()[0] + self.categ.GetWidth() + 25, self.categ.GetLocalPosition()[1])
	...
	funclist = [
		ui.__mem_func__(self.mainWeaponWindow.Show), # EQUIPEMENT
		ui.__mem_func__(self.costumePageWindow.Show), # COSTUMES
		ui.__mem_func__(self.costumePageWindow.Show), # COMPANY
		ui.__mem_func__(self.mainChestWindow.Show), # CHESTS
		ui.__mem_func__(self.mainBossWindow.Show), # BOSSES
		# ui.__mem_func__(self.mainBossWindow.Show), # MONSTERS
		ui.__mem_func__(self.mainBossWindow.Show), # METINSTONES
		# ui.__mem_func__(self.customPageWindow.LoadFile), # SYSTEMS
		ui.__mem_func__(self.customPageWindow.LoadFile), # DUNGEONS
		# ui.__mem_func__(self.customPageWindow.LoadFile), # EVENTS
		ui.__mem_func__(self.customPageWindow.LoadFile), # GUIDES
		ui.__mem_func__(self.customPageWindow.LoadFile), # PERSO
	]

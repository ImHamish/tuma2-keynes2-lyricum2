...
class CMouseController(object):
		...
		try:
			...
			if Type == player.SLOT_TYPE_SKILL:
				...
				if quickSlotType == player.SLOT_TYPE_INVENTORY:
					...
					self.AttachedIconHandle = item.GetIconInstance()
					(width, height) = item.GetItemSize()
			elif app.USE_AUTO_HUNT and Type == player.SLOT_TYPE_AUTO:
	...
	def DeattachObject(self):
		...
		if self.AttachedIconHandle != 0:
			...
			elif app.ENABLE_NEW_FISH_EVENT and self.AttachedType == player.SLOT_TYPE_FISH_EVENT:
		...
		try:
			if True == self.isAttached():
				if 0 != self.AttachedIconHandle:
					if app.ENABLE_NEW_FISH_EVENT and self.GetAttachedType() == player.SLOT_TYPE_FISH_EVENT:
	...
	def Render(self):
		if True == self.isAttached():
			if 0 != self.AttachedIconHandle:
				if app.ENABLE_NEW_FISH_EVENT:
					if self.IsAttachedIconRender == True:

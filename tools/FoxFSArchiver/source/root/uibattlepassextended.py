...
class BattlePassWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.missionNameDict = {
				...
				21 : {"name" : uiscriptlocale.EXTENDET_BATTLE_PASS_TYPENAME_22, "desc" : uiscriptlocale.EXTENDET_BATTLE_PASS_TYPENAME_22_DESC_1, "desc_non_condition" : ""},
			}
			self.mapNameDict = {
			...
			self.GetChild("tab_normal").Hide()
			self.GetChild("tab_event").Hide()
			self.GetChild("tab_button_normal").Hide()
			self.GetChild("tab_button_event").Hide()
			self.GetChild("area_normal").Hide()
			self.GetChild("area_event").Hide()
	...
	def SetPremiumPage(self):
		...
		if self.battlePassGeneralInfo[2].has_key(self.BattlePassIDPremium) and self.battlePassGeneralInfo[2][self.BattlePassIDPremium]["start_time"] < app.GetGlobalTimeStamp() and self.battlePassGeneralInfo[2][self.BattlePassIDPremium]["end_time"] > app.GetGlobalTimeStamp():
			if player.GetBattlePassID() != self.BattlePassIDPremium:
				self.GetChild("UIArea").Hide()
			...
			if self.next_premium_battlepass_time != 0:
				...
				self.GetChild("PremiumInfoText").SetText(uiscriptlocale.EXTENDET_BATTLE_PASS_NO_PLANNED_PREMIUM_BATTLEPASS_TIME)
	...
	def GetTitleTextInfo(self, mission_type, mission_condition, total_value):
		if mission_condition != 0:
			...
			if mission_type in [2, 14, 15, 18, 20, 21]:
	...
	def SetMissionDescText(self, mission_type, mission_condition, total_value):
		...
		if mission_condition != 0:
			...
			if mission_type in [17, 18, 19, 21]:
	...
	def ClearReward(self, bpID):
		for k, v in self.battlePassGeneralInfo.items():
			if v.has_key(bpID):
				self.rewardTable[k - 1] = [[],[]]
	...
	def __OnClickSendPremiumTicket(self):
		if self.slotItemPos != -1:
			net.SendExtBattlePassItem(self.slotItemPos)
			self.slotItemPos = -1
			self.GetChild("ActivateTicketButton").Hide()
			self.GetChild("ticket_slot").ClearSlot(0)
	...
	def OnUpdate(self):
		...
		if self.page == "PREMIUM" and player.GetBattlePassID() != self.BattlePassIDPremium and self.isActivePremiumWindow == 1:
...
class ListBoxMissions(ui.Window):
	...
	def Close(self):
		for item in self.selected_list:
			if item.tooltipItem and item.tooltipItem.IsShow():
	...
	def OnRunMouseWheel(self, nLen):
		if self.globalParent and self.globalParent.scrollBar and self.globalParent.scrollBar.IsShow():
			if nLen > 0:
				self.globalParent.scrollBar.OnUp()
			else:
				self.globalParent.scrollBar.OnDown()

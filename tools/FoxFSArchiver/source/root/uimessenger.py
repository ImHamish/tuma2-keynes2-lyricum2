...
FRIEND = messenger.MESSENGER_GRUOP_INDEX_FRIEND
GUILD = messenger.MESSENGER_GRUOP_INDEX_GUILD
BLOCK = messenger.MESSENGER_GROUP_INDEX_BLOCK
...
class MessengerWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_MESSENGER_BLOCK:
				...
				self.GetChild("BlockFriendButton").Hide()
	...
	def OnRefreshList(self):
		...
		if app.ENABLE_WHISPER_MESSENGER_BUTTONS:
			try:
				import newconstinfo
				if newconstinfo._interface_instance:
					newconstinfo._interface_instance.RefreshWhisperMessengerButtons()

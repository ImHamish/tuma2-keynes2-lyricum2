	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 5888 + 90, 6144 + 843)
			end
		end
		...
		when 20420.chat."Reward" with pc.in_dungeon() begin
			...
			if idx >= 2120000 and idx < 2130000 then
				if d.getf(idx, "notice_world") != 1 then
					...
					d.complete(6118, 9000, 212, "meleylair_zone")
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 212 then
				pc.warp((5888 + 90) * 100, (6144 + 843) * 100)
			elseif idx >= 2120000 and idx < 2130000 then
				pc.set_warp_location(62, 5888 + 90, 6144 + 843)
		...
		when 20419.chat."Regina Meley" begin
			local lang = pc.get_language()
			...
			if goahead == 1 then
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance_meley(100, 120, 3, 4, "meleylair_zone.cooldown")
				if result == 0 then
				...
				elseif result == 1 then
					...
					if ww_flag > 0 then
						...
						d.remove_item_meley(30341, 9000, "meleylair_zone")

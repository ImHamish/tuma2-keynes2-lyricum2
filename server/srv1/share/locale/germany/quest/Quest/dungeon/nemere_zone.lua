	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 3584 + 738, 1536 + 111)
			end
		end
		...
		when 6191.kill with pc.in_dungeon() begin
			...
			if idx >= 3520000 and idx < 3530000 then
				if d.getf(idx, "floor") == 9 then
					...
					d.complete(6191, 5400, 352, "nemere_zone")
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 352 then
				pc.warp((3584 + 738) * 100, (1536 + 111) * 100)
			elseif idx >= 3520000 and idx < 3530000 then
				pc.set_warp_location(61, 3584 + 738, 1536 + 111)
		...
		when 20395.chat."Turnul lui Nemere" begin
			...
			if goahead == 1 then
				...
				say_reward(string.format(gameforge[lang].common.dungeon_6, 95))
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance(95, 110, 71175, 0, 1, "nemere_zone.cooldown")
				if result == 0 then
				...
				elseif result == 1 then
					...
					wait()
					d.remove_item(71175, 1, 0, 0, 30331, 255, 30332, 255, 0, 0, 0, 0, 0, 0, 0, 0, 5400, "nemere_zone")

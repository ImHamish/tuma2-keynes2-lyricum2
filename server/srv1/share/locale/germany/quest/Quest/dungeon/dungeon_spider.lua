	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 512 + 177, 5632 + 478)
			end
		end
		...
		when login begin
			local idx = pc.get_map_index()
			-- if idx == 217 then
				-- pc.warp((512 + 177) * 100, (5632 + 478) * 100)
			if idx >= 2170000 and idx < 2180000 then
				pc.set_warp_location(217, 512 + 177, 5632 + 478)
		...
		when 30130.chat."Regina Paianjen" begin
			...
			if goahead == 1 then
				...
				say_reward(string.format(gameforge[lang].common.dungeon_6, 50))
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance(50, 75, 30325, 0, 1, "dungeon_spider.cooldown")
				if result == 0 then

	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.exit_all_lobby(arg, 9216 + 915, 13056 + 582)
			end
		end
		...
		when 4158.kill with pc.in_dungeon() begin
			...
			if idx >= 3570000 and idx < 3580000 then
				if d.getf(idx, "floor") == 7 then
					...
					d.complete(4158, 12600, 357, "pyramide_zone")
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 357 then
				pc.warp((9216 + 915) * 100, (13056 + 582) * 100)
			elseif idx >= 3570000 and idx < 3580000 then
				pc.set_warp_location(74, 9216 + 915, 13056 + 582)
		...
		when 9331.chat."Piramida" begin
			...
			if goahead == 1 then
				...
				say_reward(string.format(gameforge[lang].common.dungeon_6, 120))
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance(120, 120, 30798, 0, 1, "pyramide_zone.cooldown")
				if result == 0 then
				...
				elseif result == 1 then
					...
					wait()
					d.remove_item(30798, 1, 0, 0, 30800, 255, 30801, 255, 30802, 255, 30803, 255, 30804, 255, 30805, 255, 12600, "pyramide_zone")

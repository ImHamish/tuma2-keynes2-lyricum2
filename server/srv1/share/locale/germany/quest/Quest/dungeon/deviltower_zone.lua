	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 5376 + 527, 512 + 597)
			end
		end
		...
		when 1093.kill with pc.in_dungeon() begin
			...
			if idx >= 660000 and idx < 670000 then
				if d.getf(idx, "floor") == 13 then
					...
					d.complete(1093, 2700, 66, "deviltower_zone")
		...
		when 20091.chat."Hey!" with pc.in_dungeon() begin
			local idx = pc.get_map_index()
			if idx >= 660000 and idx < 670000 then
		...
		when 1092.kill with pc.in_dungeon() begin
			...
			if idx >= 660000 and idx < 670000 then
				if d.getf(idx, "floor") == 6 then
					...
					local reward = {20091, 20091, 20091}
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 66 then
				pc.warp((5376 + 527) * 100, (512 + 597) * 100)
			elseif idx >= 660000 and idx < 670000 then
				pc.set_warp_location(65, 5376 + 527, 512 + 597)
		...
		when 20348.chat."Turnul Demonilor" begin
			local lang = pc.get_language()
			...
			if goahead == 1 then
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance(40, 110, 0, 0, 1, "deviltower_zone.cooldown")
				if result == 0 then
				...
				elseif result == 1 then
					...
					wait()
					d.remove_item(0, 0, 0, 0, 50084, 255, 30302, 255, 30303, 255, 30304, 255, 0, 0, 0, 0, 2700, "deviltower_zone")

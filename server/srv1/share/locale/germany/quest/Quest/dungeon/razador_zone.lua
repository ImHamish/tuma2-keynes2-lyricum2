	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 5888 + 251, 6144 + 928)
			end
		end
		...
		when 6091.kill with pc.in_dungeon() begin
			...
			if idx >= 3510000 and idx < 3520000 then
				if d.getf(idx, "floor") == 8 then
					...
					d.complete(6091, 4500, 351, "razador_zone")
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 351 then
				pc.warp((5888 + 251) * 100, (6144 + 928) * 100)
			elseif idx >= 3510000 and idx < 3520000 then
				pc.set_warp_location(62, 5888 + 251, 6144 + 928)
		...
		when 20394.chat."Purgatoriul lui Razador" begin
			...
			if goahead == 1 then
				...
				myname = pc.get_name()
				result, cooldown, name = d.check_entrance(90, 110, 71174, 0, 1, "razador_zone.cooldown")
				if result == 0 then
				...
				elseif result == 1 then
					...
					wait()
					d.remove_item(71174, 1, 0, 0, 30329, 255, 30330, 255, 0, 0, 0, 0, 0, 0, 0, 0, 4500, "razador_zone")

quest pda_9th_quest begin
	state start begin
		function GetSkillList(lang, min_level)
			local skill_list = special.th9_active_skill_list[pc.get_job()+1][pc.get_skill_group()]
			local vnum_list = {}
			local name_list = {}
			for i = 1,table.getn(skill_list) do
				...
				if skill_level >= min_level and skill_level < 40 then
					...
					if lang == "de" then
						table.insert(name_list, locale.TH9_SKILL_NAME_DICT_DE[skill_vnum])
					elseif lang == "it" then
						table.insert(name_list, locale.TH9_SKILL_NAME_DICT_IT[skill_vnum])
					elseif lang == "ro" then
						table.insert(name_list, locale.TH9_SKILL_NAME_DICT_RO[skill_vnum])
					else
						table.insert(name_list, locale.TH9_SKILL_NAME_DICT_EN[skill_vnum])
					end
				end
			end
		...
		when 50576.use begin
			...
			say_title(string.format("%s:", item_name(50576)))
			say("")
			...
			local cooldown = pc.getf("pda_9th_quest", "cooldown") - get_global_time()
			if cooldown > 0 then
			...
			local vnum_list, name_list = pda_9th_quest.GetSkillList(lang, 30)
			...
			pc.setf("pda_9th_quest", "cooldown", get_global_time() + 21600)
			pc.remove_item(50576, 1)
		end
	end
end

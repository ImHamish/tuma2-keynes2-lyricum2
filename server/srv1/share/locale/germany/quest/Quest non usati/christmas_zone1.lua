	...
	state start begin
		function clear(arg)
			...
			if d.find(arg) then
				...
				d.kill_all(arg)
				d.clear_regen(arg)
				d.exit_all_lobby(arg, 3584 + 777, 1536 + 598)
			end
		end
		...
		when login begin
			local idx = pc.get_map_index()
			if idx == 26 then
				pc.warp((3584 + 777) * 100, (1536 + 598) * 100)
			elseif idx >= 250000 and idx < 260000 then
				pc.set_warp_location(61, 3584 + 777, 1536 + 598)
		...
		when 9293.chat."Frozen Village(easy)" with pc.get_map_index() == 61 and game.get_event_flag("christmas_event") == 1 begin
			local lang = pc.get_language()

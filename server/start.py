...
def staInit():
	...
	for dic1 in mList:
		...
		elif dic1["type"]==M2TYPE.AUTH1 or dic1["type"]==M2TYPE.AUTH2 or dic1["type"]==M2TYPE.AUTH3 or dic1["type"]==M2TYPE.AUTH4 or dic1["type"]==M2TYPE.AUTH5 or dic1["type"]==M2TYPE.AUTH6 or dic1["type"]==M2TYPE.CORE:
		...
		for dic1 in tmpProcList:
			...
			if v_system in ("FreeBSD", "Linux"):
				if not fShell("""ps afx | fgrep "./%s" | fgrep -v grep | awk '{print $1}'"""%dic1["name"], True):
					...
					#VALGRIND
					#fShell("valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=valgrind-out.txt ./%s %s &" % (dic1["name"], addFlags))

					#ASAN
					#fShell("env ASAN_OPTIONS=disable_coredump=0:abort_on_error=1:sleep_before_dying=10 ./%s %s &" % (dic1["name"], addFlags))

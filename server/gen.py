...
def WriteChannelConfig(szConfFile, dwChannel):
	if dwChannel==M2TYPE.NOCHAN:
		dwChannel=1
	elif dwChannel==M2TYPE.NOCHAN1:
		dwChannel=1
	elif dwChannel==M2TYPE.NOCHAN2:
		dwChannel=2
	elif dwChannel==M2TYPE.NOCHAN3:
		dwChannel=3
	elif dwChannel==M2TYPE.NOCHAN4:
		dwChannel=4
	elif dwChannel==M2TYPE.NOCHAN5:
		dwChannel=5
	elif dwChannel==M2TYPE.NOCHAN6:
		dwChannel=6
	
	Append2File("CHANNEL: %u" % (dwChannel), "%s" % (szConfFile))

def WriteChannelConfigSpec(szConfFile, dwChannel):
	if dwChannel==M2TYPE.NOCHAN:
		dwChannel=1
...
def WritePortConfig(szConfFile, wGenPort, dwType, dwPortType=None):
	...
	elif dwType==M2TYPE.CORE or dwType==M2TYPE.AUTH1 or dwType==M2TYPE.AUTH2 or dwType==M2TYPE.AUTH3 or dwType==M2TYPE.AUTH4 or dwType==M2TYPE.AUTH5 or dwType==M2TYPE.AUTH6:
...
def genMain(oSub={}, szParentName=[]):
	...
	for v1 in oSub:
		...
		if v1["type"]==M2TYPE.DB:
			...
			genMain(v1["sub"], listTmpParentName)
		elif v1["type"]==M2TYPE.AUTH1 or v1["type"]==M2TYPE.AUTH2 or v1["type"]==M2TYPE.AUTH3 or v1["type"]==M2TYPE.AUTH4 or v1["type"]==M2TYPE.AUTH5 or v1["type"]==M2TYPE.AUTH6 or v1["type"]==M2TYPE.CORE:
			k1s=genConfig["active"]
			#logs make paths
			CreateFolder("%s/%s/%s/cores" % (k1s, v_logFolder, szTmpParentName))
			CreateFolder("%s/%s/%s/log" % (k1s, v_logFolder, szTmpParentName))
			...
			if v1["type"]==M2TYPE.AUTH1 or v1["type"]==M2TYPE.AUTH2 or v1["type"]==M2TYPE.AUTH3 or v1["type"]==M2TYPE.AUTH4 or v1["type"]==M2TYPE.AUTH5 or v1["type"]==M2TYPE.AUTH6:
			...
			#start/logs track
			go = 0
			if v1["type"]==M2TYPE.AUTH1:
				kh1=M2TYPE.NOCHAN1
				go = 1
			elif v1["type"]==M2TYPE.AUTH2:
				kh1=M2TYPE.NOCHAN2
				go = 1
			elif v1["type"]==M2TYPE.AUTH3:
				kh1=M2TYPE.NOCHAN3
				go = 1
			elif v1["type"]==M2TYPE.AUTH4:
				kh1=M2TYPE.NOCHAN4
				go = 1
			elif v1["type"]==M2TYPE.AUTH5:
				kh1=M2TYPE.NOCHAN5
				go = 1
			elif v1["type"]==M2TYPE.AUTH6:
				kh1=M2TYPE.NOCHAN6
				go = 1
			#vType = v1["type"]
			#if v1["type"]==M2TYPE.AUTH1 or\
			#	v1["type"]==M2TYPE.AUTH2 or\
			#	v1["type"]==M2TYPE.AUTH3 or\
			#	v1["type"]==M2TYPE.AUTH4 or\
			#	v1["type"]==M2TYPE.AUTH5 or\
			#	v1["type"]==M2TYPE.AUTH6:
			#	kh1 = 0
			#	vType = 0
			else:
				go = 0
			...
			if not go:
				WriteChannelConfigSpec("%s/CONFIG"%szTmpParentName, kh1)
			else:
				WriteChannelConfig("%s/CONFIG"%szTmpParentName, kh1)
			
			if not v1["type"]==M2TYPE.AUTH1 and not v1["type"]==M2TYPE.AUTH2 and not v1["type"]==M2TYPE.AUTH3 and not v1["type"]==M2TYPE.AUTH4 and not v1["type"]==M2TYPE.AUTH5 and not v1["type"]==M2TYPE.AUTH6:

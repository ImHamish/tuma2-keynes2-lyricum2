...
void CEffectManager::Render()
{
	...
	else
	{
		...
		float fShowRange = CPythonConfig::Instance().GetFloat(CPythonConfig::EClassTypes::CLASS_OPTION, "perf_effect_range");
		...
		for (const auto& [k, effect] : m_kEftInstMap)
		{
			if (fShowRange)
			{
				const D3DXMATRIX& rkEffMat = effect->GetGlobalMatrix();
		...
		std::sort(std::execution::par, s_kVct_pkEftInstSort.begin(), s_kVct_pkEftInstSort.end(), CEffectManager_LessEffectInstancePtrRenderOrder());

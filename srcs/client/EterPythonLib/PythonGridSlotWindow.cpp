...
void CGridSlotWindow::OnRenderPickingSlot()
{
	...
	if (GetPickedSlotList(byWidth, byHeight, &SlotList))
	{
		...
		if (CheckMoving(dwSlotNumber, dwItemIndex,
#if defined(ENABLE_NEW_FISH_EVENT)
SlotListForCheck
#else
SlotList
		...
		RECT SecondRect;
		SecondRect.left = m_rect.right;
		SecondRect.top = m_rect.bottom;
		SecondRect.right = 0;
		SecondRect.bottom = 0;

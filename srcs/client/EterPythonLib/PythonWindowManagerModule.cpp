...
#if defined(ENABLE_NEW_GAMEOPTION) || defined(ENABLE_TRACK_WINDOW)
...
PyObject* wndMgrSetSlotDiffuseColor(PyObject* poSelf, PyObject* poArgs) {
	...
	D3DXCOLOR diffusecolor;
	switch (color) {
		case UI::COLOR_TYPE_GREEN:
			{
				diffusecolor = D3DXCOLOR(0.2f, 0.8f, 0.0f, 1.0f);
				break;
			}
		default:
			{
	...
	UI::CSlotWindow * pSlotWin = (UI::CSlotWindow *)pWin;
	pSlotWin->SetSlotDiffuseColor(index, diffusecolor);
	return Py_BuildNone();
}
...
PyObject* wndMgrSetSlotTrafficLight(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWin;
	if (!PyTuple_GetWindow(poArgs, 0, &pWin))
		return Py_BuildException();
	...
	pSlotWin->SetSlotTrafficLight(iSlotIndex, yLight);
	return Py_BuildNone();
}

PyObject* wndMgrDisableSlotTrafficLight(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWin;
	if (!PyTuple_GetWindow(poArgs, 0, &pWin))
		return Py_BuildException();
	...
	UI::CSlotWindow* pSlotWin = (UI::CSlotWindow*)pWin;
	pSlotWin->DisableSlotTrafficLight(iSlotIndex, yLight);
	return Py_BuildNone();
}
#endif
...
PyObject * wndMgrSetPickedAreaRender(PyObject * poSelf, PyObject * poArgs) {
	...
	int32_t flag;
	if (!PyTuple_GetInteger(poArgs, 1, &flag)) {
		return Py_BuildException();
	}
...
//		TraceError("wndMgr.SetPickedAreaRender : not a grid window");
	...
	UI::CGridSlotWindow * pGridSlotWin = (UI::CGridSlotWindow *)pWin;
	pGridSlotWin->SetPickedAreaRender(flag ? true : false);
...
PyObject * wndMgrSetDisableDeattach(PyObject * poSelf, PyObject * poArgs) {
	int32_t flag;
	if (!PyTuple_GetInteger(poArgs, 0, &flag)) {
		return Py_BuildException();
	}

	UI::CWindowManager::Instance().SetDisableDeattach(flag ? true : false);
	return Py_BuildNone();
}
...
PyObject* wndSetAniImgScale(PyObject* poSelf, PyObject* poArgs) {
	...
	((UI::CAniImageBox*)pWindow)->SetAniImgScale(x, y);
...
#endif
#if defined(ENABLE_SLOT_COVER_TRAFFIC_LIGHTS)
		{ "SetSlotTrafficLight", wndMgrSetSlotTrafficLight, METH_VARARGS },
		{ "DisableSlotTrafficLight", wndMgrDisableSlotTrafficLight, METH_VARARGS },
#endif

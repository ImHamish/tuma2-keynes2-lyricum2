...
void CIME::FinalizeString(bool bSend)
{
	...
	if (ms_bUILessMode)
	{
		// For some reason ImmNotifyIME doesn't work on DaYi and Array CHT IMEs. Cancel composition std::string by setting zero-length string.
...
void CIME::ReadingProcess(HIMC hImc)
{
    ...
    if (_GetReadingString)
	{
        ...
        // IMEs that doesn't implement Reading std::string API
		wchar_t* temp = NULL;
		DWORD tempLen = 0;

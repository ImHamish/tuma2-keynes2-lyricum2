...
void CWikiRenderTarget::RenderModel() const
{
	...
	const auto fog = state_manager.GetRenderState(D3DRS_FOGENABLE);
	...
	state_manager.SetRenderState(D3DRS_FOGENABLE, fog);
}
...
void CWikiRenderTarget::SetAura(uint32_t vnum) {
	if (!m_pModel) {
		return;
	}

					...
					else
					{
						...
						else if (ret == TEXT_TAG_EMOJI_END)
						{
							...
							if (CResourceManager::Instance().IsFileExist(szPath, __FUNCTION__)) {
								...
								TraceError("%s cannot be founded.", szPath);
							}
...
void CGraphicTextInstance::Render(RECT* pClipRect)
{
...

	{
...
#ifdef INGAME_WIKI
		m_startPos = __max(0, m_startPos);
		m_endPos = __min(m_endPos, WORD(m_pCharInfoVector.size()));
		...
		if (m_isOutline)
		{
			...
			{
				int curPos = __min(CIME::GetCurPos(), m_pCharInfoVector.size());
				if (curPos < m_startPos)
					m_startPos = __max(curPos, 0);
				else if (curPos > m_endPos)
				{
					m_endPos = curPos;
					m_startPos = __min(m_endPos, WORD(m_pCharInfoVector.size()) - 1);
					fCurX = 0;
...
#endif
		{
			...
			fFontWidth = float(pCurCharInfo->width);
			fFontHeight = float(pCurCharInfo->height);
			fFontMaxHeight = __max(fFontHeight, pCurCharInfo->height);
			fFontAdvance = float(pCurCharInfo->advance);

...
extern DWORD g_dwUpdateFPS;
extern float GetMaxLoadingDistanceSqr();
...
void CArea::RenderEffect()
{
	...
	std::sort(std::execution::par, m_kVct_pkEftInstSort.begin(), m_kVct_pkEftInstSort.end(), CArea_LessEffectInstancePtrRenderOrder());
...
void CArea::__UpdateEffectList(D3DXVECTOR3& v3Player)
{
	...
	const int iUpdateFPS = MAX(2, g_dwUpdateFPS);
	const float fMaxDist = GetMaxLoadingDistanceSqr();

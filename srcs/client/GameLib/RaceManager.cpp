...
void CRaceManager::SetRaceHeight(uint32_t vnum, float fHeight, float fRiding, float fScaleZ)
{
	TRaceHeight tb;
	tb.fHeight = fHeight;
	tb.fRiding = fRiding;
	...
	m_kMap_iRaceKey_fRaceAdditionalHeight.insert(std::map<uint32_t, TRaceHeight>::value_type(vnum, tb));
}

float CRaceManager::GetRaceHeight(uint32_t vnum)
{
	auto it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(vnum);
...
float CRaceManager::GetRidingRaceHeight(uint32_t vnum)
{
	auto it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(vnum);
	if (m_kMap_iRaceKey_fRaceAdditionalHeight.end() == it)
	{
...
float CRaceManager::GetRaceScale(uint32_t vnum)
{
	auto it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(vnum);
	if (m_kMap_iRaceKey_fRaceAdditionalHeight.end() == it)
	{

...
class CItemData
{
	public:
		enum
		{
			...
			ITEM_SOCKET_MAX_NUM			= USE_SOCKET_MAX_NUM,
#else
		...
		enum EUseSubTypes
		{
			...
			USE_SPECIAL_ENCHANT =				50,
#endif
#if defined(USE_AURA_ATTR_REMOVER)
			USE_AURA_ATTRREMOVER =				51,
#endif
			USE_REMOVE_ACCESSORY_SOCKETS = 52,
		};
...
#endif
#if defined(ENABLE_PETSKIN)
			WEAR_COSTUME_PETSKIN = 26,
#endif
#if defined(ENABLE_MOUNTSKIN)
			WEAR_COSTUME_MOUNTSKIN = 27,
#endif
#ifdef ENABLE_COSTUME_EFFECT
		...
		enum EItemWearableFlag {
			...
			WEARABLE_TALISMAN_ICE		= (1 << 13),
			WEARABLE_TALISMAN_FIRE		= (1 << 14),
			WEARABLE_TALISMAN_WIND		= (1 << 15),
			WEARABLE_TALISMAN_DARK		= (1 << 16),
			WEARABLE_TALISMAN_EARTH		= (1 << 17),
			WEARABLE_TALISMAN_ELEC		= (1 << 18),
#endif
		};
		...
		enum EApplyTypes
		{
			...
			APPLY_PC_UNUSED01,		//76
			APPLY_PC_UNUSED02,		//77
...
#pragma pack(pop)
	...
	public:
		...
		uint32_t GetAttr67MaterialVnum() const;
#endif
		/////
	...
	public:
		void SetAuraScaleTableData(uint8_t, uint8_t, float, float, float, float);
		D3DXVECTOR3& GetAuraMeshScaleVector(uint8_t, uint8_t);
		float GetAuraParticleScale(uint8_t, uint8_t);

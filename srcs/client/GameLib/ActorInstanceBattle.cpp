...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData) {
	...
	if (bAdjust == false || uiSkill != 0 || m_fAtkSpd < 1.3) {
		return 0.0f;
	}

	const auto c_dScale = (m_fAtkSpd - 1.3) / 1.3;
	const auto c_dInvTime = c_rAttackData.fInvisibleTime * 0.5;
	return c_dScale * c_dInvTime;
}

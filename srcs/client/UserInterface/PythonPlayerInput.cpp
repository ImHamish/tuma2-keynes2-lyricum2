...
void CPythonPlayer::PickCloseItem()
{
    ...
    DWORD dwItemID;
    auto& itemMgr = CPythonItem::Instance();
#ifdef USE_PICKUP_INCREASED_RANGE
    if (!itemMgr.GetCloseItem(kPPosMain, &dwItemID, 900))
#else
    if (!itemMgr.GetCloseItem(kPPosMain, &dwItemID, 600))
#endif
    {
        return;
    }
...
void CPythonPlayer::PickAllCloseItems()
{
    static uint32_t s_dwNextPickupTime = 0;
    uint32_t dwCurTime = ELTimer_GetMSec();
    if (dwCurTime >= s_dwNextPickupTime)
    {
        CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
        if (!pkInstMain)
        {
        ...
        s_dwNextPickupTime = dwCurTime + 100;

        TPixelPosition kPPosMain;
        pkInstMain->NEW_GetPixelPosition(&kPPosMain);

        std::vector<uint32_t> itemVidList;

        CPythonItem& rkItem = CPythonItem::Instance();
        if (!rkItem.GetAllCloseItems(pkInstMain->GetNameString(), kPPosMain, itemVidList))
        {
            return;
        }

        for (auto const vid : itemVidList)
        {
            SendClickItemPacket(vid, true);
        }
    }
}
#endif
...
{
#ifdef BLOCK_AUTO_ATTACK_PLAYER

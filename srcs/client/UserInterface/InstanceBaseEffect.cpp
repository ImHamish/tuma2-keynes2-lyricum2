...
bool CInstanceBase::ProcessDamage()
{
	...
	if ((flag & DAMAGE_DODGE) 
		|| (flag & DAMAGE_BLOCK))
	{
		rkEftMgr.CreateEffect(ms_adwCRCAffectEffect[bSelf ? EFFECT_DAMAGE_MISS : EFFECT_DAMAGE_TARGETMISS],v3Pos,v3Rot);
		return true;
	}
	...
	else
	{
		...
		if (bSelf)
		{
			rdwCRCEft = m_bDamageEffectType == 0 ? EFFECT_DAMAGE_SELFDAMAGE : EFFECT_DAMAGE_SELFDAMAGE2;

#if defined(USE_DMG_RENEWAL)
			if (IS_SET(flag, DAMAGE_CRITICAL) 
				&& IS_SET(flag, DAMAGE_PENETRATE))
			{
	...
	while (damage > 0) {
		...
		textures.push_back("d:/ymir work/effect/affect/damagevalue/"+ strDamageType + std::to_string(num) + ".dds");
...
#if defined(USE_DMG_RENEWAL)
		D3DXMatrixTranslation(&matTrans, FONT_WIDTH * multiplier, 0, 0);
#else
...
UINT CInstanceBase::GetNameColorIndex()
{
	if (IsPC())
	{
		IAbstractPlayer& rPlayerbr = IAbstractPlayer::GetSingleton();
		if (rPlayerbr.IsPartyMemberByVID(GetVirtualID()))
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	if (IsPC() && level >= 150)
	{
		s_kLevelColor = D3DXCOLOR(191.0f / 255.0f, 217.0f / 255.0f, 255.0f / 255.0f, 1.0f);
	}
	else
	{
	...
	if (IsNewPet())
	{
		...
		CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, D3DCOLOR_XRGB(255, 255, 0));
	}
	else
	{
#endif
...
void CInstanceBase::__Warrior_SetGeomgyeongAffect(bool isVisible)
{
	if (isVisible)
	{
		...
		m_GraphicThingInstance.SetReachScale(1.2f);
#else

...
class CPythonMiniMap : public CScreen, public CSingleton<CPythonMiniMap>
{
	...
	protected:
		...
		CGraphicExpandedImageInstance	m_BossMarkAlive;
		CGraphicExpandedImageInstance	m_BossMarkDead;
		...
		std::vector<SBossPositionInfo> m_AtlasBossMark;
	public:
		void ClearBossMarkInfo();
		void RegisterBossAtlasInfo(uint32_t vnum, int32_t x, int32_t y, uint32_t time, uint32_t real_time, bool is_alive);
		bool GetBossAtlasInfo(float x, float y, uint32_t* vnum, uint32_t* respawn_time, uint32_t* real_time, bool* is_alive);
};
...
void CPythonApplication::OnMouseWheel(int nLen)
{
	if (!(UI::CWindowManager::Instance().RunMouseWheelEvent(nLen)))
	{
		...
		if (pkCmrCur) {
			pkCmrCur->Wheel(nLen);
#ifdef ENABLE_SAVECAMERA_PREFERENCES
			if (CCameraManager::DEFAULT_PERSPECTIVE_CAMERA == rkCmrMgr.GetCurrentCameraNum()) {
				CPythonSystem::Instance().SetCameraHeight(pkCmrCur->GetDistance());
			}
#endif
		}
	}
...
void CPythonApplication::OnKeyDown(int iIndex)
{
	...
	if (rkInstance.DirectEnterMode_IsSet() || 
		rkInstance.IsLoadingPhase()) {
		return;
	}

...
static uint32_t dwCloseItemDistance = 1000 * 1000;
...
bool CPythonItem::GetCloseItem(const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance)
{
	...
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		...
		if (dwxDistance*dwxDistance + dwyDistance*dwyDistance < dwCloseItemDistance)
		{
            ...
            if (!CPythonPlayer::Instance().CanPickItem(i->second->dwVirtualNumber))
            {
                continue;
            }
#endif
...
#ifdef USE_QUICK_PICKUP
bool CPythonItem::GetAllCloseItems(const std::string& myName, const TPixelPosition& c_rPixelPosition, std::vector<uint32_t>& itemVidList)
{
    for (auto const& pair : m_GroundItemInstanceMap)
    {
        uint32_t dwxDistance = uint32_t(c_rPixelPosition.x - pair.second->v3EndPosition.x);
        uint32_t dwyDistance = uint32_t(c_rPixelPosition.y - (-pair.second->v3EndPosition.y));
        uint32_t dwDistance = uint32_t(dwxDistance * dwxDistance + dwyDistance * dwyDistance);

        if (dwDistance < dwCloseItemDistance && (pair.second->stOwnership == "" || pair.second->stOwnership == myName))
        {
            if (itemVidList.size() == 50)
            {
                break;
            }

#ifdef USE_PICKUP_FILTER
            if (!CPythonPlayer::Instance().CanPickItem(pair.second->dwVirtualNumber))
            {
                continue;
            }
#endif

            itemVidList.push_back(pair.first);
        }
    }

...
float GetMaxLoadingDistanceSqr()
{
	int peNum;
	float pfStart, pfEnd, pfFarClip;
	CPythonBackground::instance().GetDistanceSetInfo(&peNum, &pfStart, &pfEnd, &pfFarClip);

	float fRatio = CPythonConfig::Instance().GetFloat(CPythonConfig::EClassTypes::CLASS_OPTION, "perf_loading_dist");
...
{
	static std::vector<std::string> mapVec{

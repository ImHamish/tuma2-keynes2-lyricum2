...
/*
    if (pkInstMain->IsMountingHorse() && GetSkillGrade(104) < 1)
    {
        return false;
    }
*/
...
bool CPythonPlayer::__UseSkill(DWORD dwSlotIndex)
{
		...
		pSkillData->IsAutoSearchTarget())
	{
		...
		if (pkInstTarget)
		{
			if ((rkSkillInst.dwIndex == 109 || pSkillData->IsOnlyForAlliance()) && (pkInstTarget->IsNPC() ||
			...
			{
				SetTarget(pkInstMain->GetVirtualID(), true);
			}

			if (rkSkillInst.dwIndex == 109 || pSkillData->IsOnlyForAlliance())
			{
	...
	if (pSkillData->CanChangeDirection())
	{
				...
				!pkInstTarget->IsStoneDoor())))
		{
			...
			{
				Tracenf("CPythonPlayer::__UseSkill(%d) - Θ­?ι ±βΑΨ ?ζΗβ ?³Α¤?≫ ΗΨ??ΗΤ", dwSlotIndex);
			}
		}
	}

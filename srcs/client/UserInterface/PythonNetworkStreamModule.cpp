...
PyObject* netUploadMark(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SetMarkImageFileName(szFileName);
	rkNetStream.SendGuildChangeSymbol();
...
PyObject* netSendCreateCharacterPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	char* country_flag;
	if (!PyTuple_GetString(poArgs, 8, &country_flag))
		return Py_BuildException();
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendCreateCharacterPacket((BYTE) index, name, (BYTE) job, (BYTE) shape, stat1, stat2, stat3, stat4, country_flag);
	return Py_BuildNone();
}
...
PyObject* netSendItemDropPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemDropPacket(Cell);
	return Py_BuildNone();
}
...
PyObject* netSendItemDropPacketNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemDropPacketNew(Cell, count);
	return Py_BuildNone();
}
...
PyObject* netSendSwitchBotChange(PyObject* poSelf, PyObject* poArgs)
{
	...
	int vnum_change;
	if (!PyTuple_GetInteger(poArgs, 1, &vnum_change))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendSwitchBotChange(attr,vnum_change);
	return Py_BuildNone();
}
#endif
...
PyObject* netSetChannelNumber(PyObject* poSelf, PyObject* poArgs) {
	BYTE ch;
	if (!PyTuple_GetInteger(poArgs, 0, &ch)) {
		return Py_BuildException();
	}

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SetChannelNumber(ch);
	return Py_BuildNone();
}
...
PyObject* netSendChangeChannel(PyObject* poSelf, PyObject* poArgs) {
	BYTE ch;
	if (!PyTuple_GetInteger(poArgs, 0, &ch)) {
		return Py_BuildException();
	}
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChangeChannel(ch);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendExchangeFragmentsPacket(PyObject* poSelf, PyObject* poArgs)
{
	int window;
	...
	CPythonNetworkStream::Instance().SendExchangeFragmentsPacket(TItemPos(window, slot));
	return Py_BuildNone();
}
#endif
...
PyObject* netSendReqMapBossInfo(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendReqMapBossInfo();
	return Py_BuildNone();
}
...
PyObject* netSendFishBoxUse(PyObject* poSelf, PyObject* poArgs) {
	...
	CPythonNetworkStream::Instance().SendFishBoxUse(window, pos);
	return Py_BuildNone();
}
...
PyObject* netSendFishShapeAdd(PyObject* poSelf, PyObject* poArgs) {
	...
	CPythonNetworkStream::Instance().SendFishShapeAdd(pos);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendMessengerAddBlockByVIDPacket(PyObject* poSelf, PyObject* poArgs) {
	...
	CPythonNetworkStream::Instance().SendMessengerAddBlockByVIDPacket(vid);
...
PyObject* netSendMessengerAddBlockByNamePacket(PyObject* poSelf, PyObject* poArgs) {
	...
	CPythonNetworkStream::Instance().SendMessengerAddBlockByNamePacket(szName);
...
PyObject* netSendMessengerBlockRemovePacket(PyObject* poSelf, PyObject* poArgs) {
	...
	CPythonNetworkStream::Instance().SendMessengerRemoveBlockPacket(szKey, szName);
...
PyObject* netSendTakeLettersEventReward(PyObject* poSelf, PyObject* poArgs) {
	CPythonNetworkStream::Instance().SendTakeLettersEventReward();
	return Py_BuildNone();
}
#endif
...
PyObject* netSendMultiItemUsePacket(PyObject* poSelf, PyObject* poArgs)
{
	uint16_t count = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &count))
	{
		return Py_BuildException();
	}
	...
	CPythonNetworkStream::Instance().SendMultiItemUsePacket(count, Cell);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendExtBattlePassItem(PyObject* poSelf, PyObject* poArgs)
{
	int slotindex = 0;
	...
	CPythonNetworkStream::Instance().SendExtBattlePassItem(slotindex);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendMultipleOpening(PyObject*/* poSelf*/, PyObject* poArgs)
{
    TItemPos pos;
    if (!PyTuple_GetByte(poArgs, 0, &pos.window_type) || !PyTuple_GetUnsignedShort(poArgs, 1, &pos.cell))
    {
        return Py_BadArgument();
    }

    uint16_t count;
    if (!PyTuple_GetUnsignedShort(poArgs, 2, &count))
    {
        return Py_BadArgument();
    }

    auto& netMgr = CPythonNetworkStream::Instance();
    netMgr.SendMultipleOpening(pos, count);
    return Py_BuildNone();
}
#endif

PyObject* netSendChangeCountryFlagPacket(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();
	char* szCountryFlag;
	if (!PyTuple_GetString(poArgs, 1, &szCountryFlag))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChangeCountryFlagPacket((BYTE)iIndex, szCountryFlag);
	return Py_BuildNone();
}

PyObject* netSendReqWhisperInfo(PyObject* poSelf, PyObject* poArgs)
{
	char* name;
...
void initnet()
{
	...
	{
#if defined(ENABLE_MULTI_ITEM_USE)
		{"a5f37iuhfs55539141", netSendMultiItemUsePacket, METH_VARARGS},
#endif
#if defined(ENABLE_AURA_SYSTEM)
...
#endif
#if defined(ENABLE_LETTERS_EVENT)
		{"ajf32iuhfs55539141", netSendTakeLettersEventReward, METH_VARARGS},
#endif
#if defined(ENABLE_MESSENGER_BLOCK)
...
#endif
#if defined(ENABLE_NEW_FISH_EVENT)
		{"ajf37iuhfs55539141", netSendFishBoxUse, METH_VARARGS},
		{"ajf36iuhfs55539141", netSendFishShapeAdd, METH_VARARGS},
#endif
#ifdef USE_CAPTCHA_SYSTEM
        {"ajf35iuhfs55539141", netSendConfirmCaptcha, METH_VARARGS},
#endif
#if defined(ENABLE_EXCHANGE_FRAGMENTS)
		{"ajf34iuhfs55539141", netSendExchangeFragmentsPacket, METH_VARARGS},
#endif
#if defined(ENABLE_GAYA_RENEWAL)
		...
		{"SendChangeChannel", netSendChangeChannel, METH_VARARGS},
#endif
		...
		{ "SendChangeCountryFlagPacket", netSendChangeCountryFlagPacket, METH_VARARGS },
		{ "SendReqWhisperInfo", netSendReqWhisperInfo, METH_VARARGS },
...
*/
		...
		{"SendExtBattlePassItem", netSendExtBattlePassItem, METH_VARARGS},
#endif
#if defined(USE_ATTR_6TH_7TH)
...
#endif
#ifdef USE_MULTIPLE_OPENING
        { "SendMultipleOpening", netSendMultipleOpening, METH_VARARGS },
#endif
		{ "SendReqMapBossInfo",					netSendReqMapBossInfo,					METH_VARARGS },
		{ NULL,										NULL,										NULL },
	};
	...
	PyModule_AddIntConstant(poModule, "ACCOUNT_CHARACTER_SLOT_COUNTRY_FLAG", CPythonNetworkStream::ACCOUNT_CHARACTER_SLOT_COUNTRY_FLAG);

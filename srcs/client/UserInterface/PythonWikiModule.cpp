...
PyObject* wikiGetRefineInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (CItemManager::instance().GetItemDataPointer(itemVnum, &table))
	{
		...
		for (int i = 0; i < refineMaxLevel; ++i)
		{
			...
			PyTuple_SetItem(ret, i, Py_BuildValue("LO", table->GetWikiTable()->pRefineData[i].price, ret2));
		}
...
PyObject* wikiUnregisterModelViewWindow(PyObject* poSelf, PyObject* poArgs)
{
	int moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID))
		return Py_BadArgument();
	...
	CPythonWikiRenderTarget::instance().UnregisterRenderModule(moduleID, hwnd);
	return Py_BuildNone();
}
...
PyObject* wikiSetModelAura(PyObject* poSelf, PyObject* poArgs) {
	int32_t moduleID;
	if (!PyTuple_GetInteger(poArgs, 0, &moduleID)) {
		return Py_BadArgument();
	}
	...
	CPythonWikiRenderTarget::instance().SetModelAura(moduleID, auraVnum);
	return Py_BuildNone();
}
#endif

PyObject* wikiIsUniqueRealItem(PyObject* poSelf, PyObject* poArgs) {
	int32_t vnum;
	if (!PyTuple_GetInteger(poArgs, 0, &vnum)) {
		return Py_BadArgument();
	}

	return Py_BuildValue("i", CItemManager::instance().IsUniqueRealItem(vnum));
}
...
void initWiki()
{
	...
	{
		{"IsUniqueRealItem", wikiIsUniqueRealItem, METH_VARARGS},
#if defined(ENABLE_AURA_SYSTEM)
		{"SetModelAura", wikiSetModelAura, METH_VARARGS},
#endif
		...
		{ "UnegisterModelViewWindow",			wikiUnregisterModelViewWindow,		METH_VARARGS },

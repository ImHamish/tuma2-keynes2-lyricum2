...
#else
	if (const CGraphicThing* pkMotionThing = pRaceData->RegisterMotionData(iMode, iMotion, c_szFullFileName, iWeight))
		CResourceManager::Instance().LoadStaticCache(pkMotionThing->GetFileName());
#endif
...
PyObject * chrmgrSetRaceHeight(PyObject* poSelf, PyObject* poArgs)
{
	...
	float fScaleZ = 0.0f;
	if (!PyTuple_GetFloat(poArgs, 3, &fScaleZ))
	{
		return Py_BadArgument();
	}

	CRaceManager::Instance().SetRaceHeight(iRaceIndex, fRaceHeight, fRidingHeight, fScaleZ);
	return Py_BuildNone();
}
#endif
...
PyObject* chrmgrSetAutoSystemByType(PyObject*/* poSelf*/, PyObject* poArgs)
{
    ...
    CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
    rkChrMgr.SetAutoSystemByType(bType, bOnOff);
    return Py_BuildNone();
}
...
PyObject* chrmgrGetAutoSystemByType(PyObject*/* poSelf*/, PyObject* poArgs)
{
    ...
    CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
    return Py_BuildValue("b", rkChrMgr.GetAutoSystemByType(bType));
}
#endif
...
void initchrmgr()
{
	...
	PyModule_AddIntConstant(poModule, "EFFECT_BP_MISSION_COMPLETED", CInstanceBase::EFFECT_BP_MISSION_COMPLETED);
	PyModule_AddIntConstant(poModule, "EFFECT_BP_COMPLETED", CInstanceBase::EFFECT_BP_COMPLETED);
#endif
#ifdef USE_AUTO_HUNT
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_START", AUTO_ONOFF_START);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_ATTACK", AUTO_ONOFF_ATTACK);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_METINSTONES", AUTO_ONOFF_METINSTONES);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_RANGE", AUTO_ONOFF_RANGE);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_POTION", AUTO_ONOFF_POTION);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_SKILL", AUTO_ONOFF_SKILL);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_RESTART", AUTO_ONOFF_RESTART);
    PyModule_AddIntConstant(poModule, "AUTO_ONOFF_MAX", AUTO_ONOFF_MAX);
#endif
}

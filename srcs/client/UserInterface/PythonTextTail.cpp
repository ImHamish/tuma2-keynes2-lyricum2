...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		CGraphicImageInstance * pGuildCrownInstance = pTextTail->pGuildCrownInstance;
		...
		if (pMarkInstance && pGuildNameInstance)
		{
			...
			if (pGuildCrownInstance)
				pGuildCrownInstance->SetPosition(pTextTail->x - (iWidth / 2) + iImageHalfSize, pTextTail->y - (c_fyMarkPosition * 1.8f));
		}
		...
		if (pTitle)
		{
			...
			if (pLevel)
			{
				...
				if (pLanguageInstance)
				{
					int iLevelWidth, iLevelHeight;
					pLevel->GetTextSize(&iLevelWidth, &iLevelHeight);
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - iLevelWidth - pLanguageInstance->GetWidth() - 4.0f, pTextTail->y - 10.0f);
				}
#endif	
			}
		}
		...
		CGraphicImageInstance* pPremium = pTextTail->pPremiumInstance;
		if (pPremium)
		{
			CGraphicTextInstance* pLevel = pTextTail->pLevelTextInstance;
			...
			if (pLevel)
			{
				...
				{
					pTitle->GetTextSize(&iTitleWidth, &iTitleHeight);
					pPremium->SetPosition(pTextTail->x - (iNameWidth / 2) - 7.0f - iTitleWidth - iLevelWidth - pPremium->GetWidth() - 20.0f, pTextTail->y - 12.0f);
				}
				if (!pTitle)
					pPremium->SetPosition(pTextTail->x - (iNameWidth / 2) - 4.0f - iLevelWidth - pPremium->GetWidth() - 20.0f, pTextTail->y - 12.0f);
...
#endif
#ifdef ENABLE_PREMIUM_PLAYERS
		if (pTextTail->pPremiumInstance)
		{
			pTextTail->pPremiumInstance->Render();
		}
#endif

		if (pTextTail->pGuildCrownInstance)
		{
			pTextTail->pGuildCrownInstance->Render();
		}
	}
...
#if defined(ENABLE_NEW_SHOP_IN_CITIES)
	if (pTextTail->bIsShop == true)
	{
		if (CPythonSystem::Instance().GetHideMode5Status())
		{
			pTextTail->bIsShow = false;
		}
		else
		{
			pTextTail->bIsShow = (pTextTail->fDistanceFromPlayer < 3500.f);
		}
	}
#endif
}
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight, bool IsPC)
{
	...
	pTextTail->pGuildCrownInstance =NULL;
#ifdef ENABLE_MULTI_LANGUAGE
	pTextTail->pLanguageInstance = NULL;
#endif
#ifdef ENABLE_PREMIUM_PLAYERS
	pTextTail->pPremiumInstance = NULL;
#endif
	...
	if (0 != dwGuildID)
	{
		...
		if (pCharacterInstance->IsPC() && pCharacterInstance->GetGuildLeaderGradeType() == 0)
		{
			CGraphicImageInstance*& pGuildCrownInstance = pTextTail->pGuildCrownInstance;
			if (!pGuildCrownInstance)
			{
				CGraphicImage* pGuildCrownImage = (CGraphicImage*)CResourceManager::Instance().GetResourcePointer("d:/ymir work/effect/etc/guild_crown/crown_10.tga");
				if (pGuildCrownImage)
				{
					pGuildCrownInstance = CGraphicImageInstance::New();
					pGuildCrownInstance->SetImagePointer(pGuildCrownImage);
				}
			}
		}
	}
	...
	if (pCharacterInstance->IsPC()) {
		...
		{
			char szFileName[256];
			sprintf(szFileName, "d:/ymir work/ui/country_flags/chat/%s.png", pCharacterInstance->GetCountryFlag().c_str());
...
#ifdef ENABLE_PREMIUM_PLAYERS
	if (pCharacterInstance->IsPC() && pCharacterInstance->IsPremiumPlayer())
	{
		pTextTail->pPremiumInstance = CGraphicImageInstance::New();
		char szPath[256];
		sprintf(szPath, "d:/ymir work/ui/tail.tga");
		pTextTail->pPremiumInstance->SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPath));
	}
#endif
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pGuildCrownInstance)
	{
		CGraphicImageInstance::Delete(pTextTail->pGuildCrownInstance);
		pTextTail->pGuildCrownInstance = nullptr;
	}

#ifdef ENABLE_PREMIUM_PLAYERS
	if (pTextTail->pPremiumInstance)
	{
		CGraphicImageInstance::Delete(pTextTail->pPremiumInstance);
		pTextTail->pPremiumInstance = NULL;
	}
#endif

...
void CPythonCharacterManager::GetMobWithVnum(uint32_t bossVnum, std::vector<CInstanceBase*>& m_Data) {
	m_Data.clear();
...
void CPythonCharacterManager::__RenderSortedAliveActorList()
{
	...
	const TCharacterInstanceMap& rkMap_pkInstAlive = m_kAliveInstMap;
...
void CPythonCharacterManager::__RenderSortedDeadActorList()
{
	...
	std::sort(std::execution::par, s_kVct_pkInstDeadSort.begin(), s_kVct_pkInstDeadSort.end(), [](CInstanceBase* pkLeft, CInstanceBase* pkRight) { return pkLeft->LessRenderOrder(pkRight); });
...
#ifdef USE_AUTO_HUNT
bool CPythonCharacterManager::GetAutoOnOff()
{
    return m_bAutoHuntOnOff;
}

void CPythonCharacterManager::SetAutoOnOff(const bool v, const bool c/* = false */)
{
    m_bAutoHuntOnOff = v;
    m_bAutoHuntCommand = c;
}

bool CPythonCharacterManager::GetAutoSystemByType(const uint8_t t)
{
    if (t >= AUTO_ONOFF_MAX)
    {
        return false;
    ...
    return m_abAutoHuntType[t];
}

void CPythonCharacterManager::SetAutoSystemByType(const uint8_t t, const bool v)
{
    if (t >= AUTO_ONOFF_MAX)
    {
        return;
    ...
    m_abAutoHuntType[t] = v;

    if (t == AUTO_ONOFF_RANGE)
    {
...
TPixelPosition CPythonCharacterManager::GetLastPixelPosition()
{
    return m_rkLastPixelPos;
}

void CPythonCharacterManager::SetLastPixelPosition(const TPixelPosition rkInstPos)
{
    m_rkLastPixelPos = rkInstPos;
}
#endif

...
PyObject * chatGetLinkFromHyperlink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (0 == results[0].compare("item")) {
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData)) {
			...
			int32_t len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x", htoi(results[2].c_str()), htoi(results[3].c_str()));

			uint8_t args = 4;

			for (uint8_t i = 0; i <= ITEM_SOCKET_SLOT_MAX_NUM; i++)
			{
				len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x", htoi(results[args].c_str()));
				args++;
			}
			...
			if (results.size() >= args) {
				for (uint8_t i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; i++) {
					...
					len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%d", type, atoi(results[args + 1].c_str()));
					args += 2;
				}
			}
...
void initChat()
{
	...
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_FRIEND_BLOCKED", CPythonChat::WHISPER_TYPE_FRIEND_BLOCKED);
#endif
}

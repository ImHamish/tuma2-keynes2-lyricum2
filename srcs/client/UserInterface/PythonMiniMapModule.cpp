...
PyObject* miniMapGetBossAtlasInfo(PyObject* /*poSelf*/, PyObject* poArgs) {
	float fScrrenX;
	if (!PyTuple_GetFloat(poArgs, 0, &fScrrenX))
		return Py_BuildException();
	...
	uint32_t vnum, respawn_time, real_time;
	bool is_alive;
	if (!CPythonMiniMap::Instance().GetBossAtlasInfo(fScrrenX, fScrrenY, &vnum, &respawn_time, &real_time, &is_alive))
		return Py_BuildNone();

	return Py_BuildValue("iiii", vnum, respawn_time, real_time, is_alive);
}
...
void initMiniMap()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetBossAtlasInfo",				miniMapGetBossAtlasInfo,						METH_VARARGS },
		{ NULL, NULL },
	};

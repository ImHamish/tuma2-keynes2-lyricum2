...
bool CPythonNonPlayer::LoadNPCWear(const char* szNPCWearFile) {
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		if ((DWORD)(TokenVector.size()) != (DWORD)(NPC_WEAR_MAX + 1))
		{
			continue;
		}

		const DWORD npcVnum = atoi(TokenVector[0].c_str());
		std::vector<DWORD> tmpVec;
		tmpVec.resize(NPC_WEAR_MAX);

		for (BYTE i = 0; i < NPC_WEAR_MAX; ++i)
		{
			tmpVec[i] = atoi(TokenVector[i + 1].c_str());
		}

/*
		if (tmpVec[NPC_WEAR_RACE] > 7)
		{
			continue;
		}
*/

		m_npcWear[npcVnum] = tmpVec;
	}
...
int CPythonNonPlayer::GetNPCWear(const int raceIndex, const BYTE wearIndex) {
	...
	const auto it = m_npcWear.find(raceIndex);
	if (it == m_npcWear.end()) {
		return -1;
	}

	return it->second[wearIndex];
}
#endif

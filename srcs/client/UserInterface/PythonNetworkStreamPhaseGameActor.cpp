...
void __SetWeaponPower(IAbstractPlayer& rkPlayer, DWORD dwWeaponID) {
	...
	CItemData* pkWeapon;
#if defined(ENABLE_WEAPON_COSTUME_SYSTEM) || defined(ENABLE_CHANGELOOK)
	uint32_t weaponVnum = CPythonPlayer::Instance().GetItemIndex(TItemPos(INVENTORY, c_Equipment_Weapon));

	if (weaponVnum > 0 && CItemManager::Instance().GetItemDataPointer(weaponVnum, &pkWeapon))
#else
...
bool CPythonNetworkStream::RecvCharacterAppendPacket()
{
	...
	kNetActorData.m_byPremium = 0;
	kNetActorData.m_iPremiumTime = 0;
#endif

	...
	if (!__IsMainInstance() && !IsAttacking())
	{
		if (IsPC() && IsWalking())
		{
			...
			for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
			{
				...
				if (pkInstEach->IsAffect(AFFECT_INVISIBILITY) && pkInstEach->IsGameMaster())
					continue;

#ifdef ENABLE_NO_COLLISION
#ifdef USE_AUTO_HUNT
                if (rkChrMgr.GetAutoOnOff() && rkChrMgr.GetAutoSystemByType(AUTO_ONOFF_ATTACK))
                {
                    bEffect = false;
                }
                else
                {
                    bEffect = pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET);
                }
#else
                bEffect = pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET);
#endif

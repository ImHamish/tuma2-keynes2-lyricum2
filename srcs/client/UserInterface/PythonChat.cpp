...
namespace
{
	bool get_country_flag_from_chat(std::string& text, std::string& country_flag)
	{
		try
		{
			static std::regex pattern(R"(\|L(.*?)\|l)");
			if (std::smatch match; std::regex_search(text, match, pattern))
			{
				country_flag = match[1];
				text = match.prefix().str() + match.suffix().str();
			}
		}
	...
	bool get_empire_flag_from_chat(std::string& text, std::string& empire_flag)
	{
		try
		{
			static std::regex pattern(R"(\|E(.*?)\|e)");
			if (std::smatch match; std::regex_search(text, match, pattern))
			{
				empire_flag = match[1];
				switch (atoi(empire_flag.c_str()))
				{
				case CInstanceBase::EMPIRE_A:
					empire_flag = "shinsoo";
					break;
				case CInstanceBase::EMPIRE_B:
					empire_flag = "chunjo";
					break;
				case CInstanceBase::EMPIRE_C:
					empire_flag = "jinno";
					break;
				default:
					empire_flag = "";
...
void CPythonChat::AppendChat(int iType, const char * c_szChat)
{
	...
	if (startLang != std::string::npos && endLang != std::string::npos)
	{
		...
		char szLangName[256];
		sprintf(szLangName, "d:/ymir work/ui/country_flags/chat/%s.png", strLanguage.c_str());
		if (CResourceManager::Instance().IsFileExist(szLangName, __FUNCTION__))
		{
			...
			if (pLanguageImage)
			{
				...
				//pChatLine->pLanguage.SetScale((2.0f / 3.0f), (1.0f / 2.0f));
			}
		}
...
//	if (iType == CHAT_TYPE_SHOUT) {
	...
	if (startEmpire != std::string::npos && endEmpire != std::string::npos) {
		...
		char empireicon[256];
		sprintf(empireicon, "d:/ymir work/ui/game/flag/%s.tga", strEmpire.c_str());

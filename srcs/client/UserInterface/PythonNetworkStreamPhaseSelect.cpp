...
void CPythonNetworkStream::SelectPhase()
{
	...
	switch (header)
	{
		...
		case HEADER_GC_CHANGE_COUNTRY_FLAG:
			if (__RecvChangeCountryFlag())
				return;
			break;
...
bool CPythonNetworkStream::SendChangeCountryFlagPacket(BYTE index, const char* c_szCountryFlag)
{
	TPacketCGChangeCountryFlag p;
	p.header = HEADER_CG_CHANGE_COUNTRY_FLAG;
	p.index = index;
	strncpy(p.country_flag, c_szCountryFlag, 3);

	if (!Send(sizeof(p), &p))
	{
#ifdef _DEBUG
		Tracen("Failed to SendChangeCountryFlagPacket");
#endif
...
bool CPythonNetworkStream::SendCreateCharacterPacket(BYTE index, const char *name, BYTE job, BYTE shape, BYTE byCON, BYTE byINT, BYTE bySTR, BYTE byDEX, const char* c_szCountryFlag)
{
	...
	strncpy(createCharacterPacket.country_flag, c_szCountryFlag, 3);
...
bool CPythonNetworkStream::__RecvChangeCountryFlag()
{
	TPacketGCChangeCountryFlag p;
	if (!Recv(sizeof(p), &p))
		return false;

	strncpy(m_akSimplePlayerInfo[p.index].country_flag, p.country_flag, 3);

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_SELECT], "OnChangeCountryFlag", Py_BuildValue("(s)", p.country_flag));
	return true;
}

...
//		AbsoluteExitGame();
    ...
    else if (!strcmpi(szCmd, "auto_onoff"))
    {
        ...
        bool bStatus = atoi(TokenVector[1].c_str()) == 1 ? true : false;

        CPythonPlayer::Instance().SetAutoUseOnOff(bStatus);

        PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetAutoStatus", Py_BuildValue("(b)", bStatus));
    }
#endif
	...
	else if (!strcmpi(szCmd, "cards"))
	{
		...
		{
			uint32_t safemode = atoi(TokenVector[2].c_str());
		...
		else if ("info" == TokenVector[1])
		{
			...
			uint32_t card_in_hand_1 = atoi(TokenVector[2].c_str());
			uint32_t card_in_hand_1_v = atoi(TokenVector[3].c_str());
			uint32_t card_in_hand_2 = atoi(TokenVector[4].c_str());
			uint32_t card_in_hand_2_v = atoi(TokenVector[5].c_str());
			uint32_t card_in_hand_3 = atoi(TokenVector[6].c_str());
			uint32_t card_in_hand_3_v = atoi(TokenVector[7].c_str());
			uint32_t card_in_hand_4 = atoi(TokenVector[8].c_str());
			uint32_t card_in_hand_4_v = atoi(TokenVector[9].c_str());
			uint32_t card_in_hand_5 = atoi(TokenVector[10].c_str());
			uint32_t card_in_hand_5_v = atoi(TokenVector[11].c_str());
			uint32_t cards_left = atoi(TokenVector[12].c_str());
			uint32_t points = atoi(TokenVector[13].c_str());
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Cards_UpdateInfo",
					Py_BuildValue("(iiiiiiiiiiii)", card_in_hand_1, card_in_hand_1_v, card_in_hand_2, card_in_hand_2_v,
													card_in_hand_3, card_in_hand_3_v, card_in_hand_4, card_in_hand_4_v,
													card_in_hand_5, card_in_hand_5_v, cards_left, points));
		}
		else if ("finfo" == TokenVector[1])
		{
			...
			uint32_t hand_1 = atoi(TokenVector[2].c_str());
			uint32_t hand_1_v = atoi(TokenVector[3].c_str());
			uint32_t hand_2 = atoi(TokenVector[4].c_str());
			uint32_t hand_2_v = atoi(TokenVector[5].c_str());
			uint32_t hand_3 = atoi(TokenVector[6].c_str());
			uint32_t hand_3_v = atoi(TokenVector[7].c_str());
			uint32_t points = atoi(TokenVector[8].c_str());
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Cards_FieldUpdateInfo",
					Py_BuildValue("(iiiiiii)", hand_1, hand_1_v, hand_2, hand_2_v, hand_3, hand_3_v, points));
		}
		else if ("reward" == TokenVector[1])
		{
			...
			uint32_t hand_3_v = atoi(TokenVector[7].c_str());
			uint32_t points = atoi(TokenVector[8].c_str());
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Cards_PutReward",
					Py_BuildValue("(iiiiiii)", hand_1, hand_1_v, hand_2, hand_2_v, hand_3, hand_3_v, points));
		}

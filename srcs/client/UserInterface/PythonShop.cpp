...
#else
void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, uint32_t dwPrice)
#endif
{
...
#else
uint32_t CPythonShop::GetPrivateShopItemPrice(TItemPos ItemPos)
#endif
{
...
#else
	uint32_t iValue = CPythonShop::Instance().GetPrivateShopItemPrice(TItemPos(bItemWindowType, wItemSlotIndex));

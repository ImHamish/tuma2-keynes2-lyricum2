...
void CInstanceBase::SHORSE::Create(const TPixelPosition& c_rkPPos, UINT eRace, UINT eHitEffect)
{
	...
	{
		float fScaleZ = CRaceManager::instance().GetRaceScale(eRace);
		if ((DWORD)(fScaleZ) != 100)
		{
			fScaleZ /= 100.0f;
			rkActor.SetScaleWorld(fScaleZ, fScaleZ, fScaleZ);
		}
	}
#endif
...
void CInstanceBase::SetPremiumPlayer(BYTE byValue)
{
	m_byPremium = byValue;
}

int CInstanceBase::GetPremiumPlayerTime()
{
	return m_iPremiumTime;
}

void CInstanceBase::SetPremiumPlayerTime(int iValue)
{
	m_iPremiumTime = iValue;
}

bool CInstanceBase::IsPremiumPlayer()
{
	bool bResult = m_byPremium != 0 ? true : false;
	return bResult;
}
#endif
...
)
{
	...
	if (npcRaceIndex != -1)
	{
		...
		float fScaleZ = CRaceManager::instance().GetRaceScale(c_rkCreateData.m_dwRace);
		if ((DWORD)(fScaleZ) != 100)
		{
			fScaleZ /= 100.0f;
			m_GraphicThingInstance.SetScaleWorld(fScaleZ, fScaleZ, fScaleZ);
		}
	}
	else
	{
		if (c_rkCreateData.m_dwMountVnum != 0)
		{
	...
	m_GraphicThingInstance.SetRealRaceIndex(c_rkCreateData.m_dwRace);
#else
	if (c_rkCreateData.m_dwMountVnum != 0)
	{
		MountHorse(c_rkCreateData.m_dwMountVnum);
...
#endif
		SetCountryFlag(c_rkCreateData.m_country_flag);
	}
...
#ifdef ENABLE_PREMIUM_PLAYERS
	SetPremiumPlayer(c_rkCreateData.m_byPremium);
	SetPremiumPlayerTime(c_rkCreateData.m_iPremiumTime);
#endif
	...
	{
		float fScaleZ = CRaceManager::instance().GetRaceScale(GetRace());
		if ((DWORD)(fScaleZ) != 100)
		{
			fScaleZ /= 100.0f;
			m_GraphicThingInstance.SetScaleWorld(fScaleZ, fScaleZ, fScaleZ);
...
#if defined(ENABLE_DAMAGE_QUEUE_RENEWAL)
	while(ProcessDamage());
#else
	ProcessDamage();
#endif
...
bool CInstanceBase::IsAttackableInstance(CInstanceBase& rkInstVictim)
{
	...
	else if (IsPC())
	{
		...
		if (rkInstVictim.IsPC())
		{
			static std::set<std::string> maps = {
			...
			std::string map_name = CPythonBackground::Instance().GetWarpMapName();
			stl_lowers(map_name);
...
bool CInstanceBase::__CanRender()
{
	...
	if (IsPC() && IsDead())
	{
		if (m_awPart[CRaceData::PART_ACCE] != 0)
		{
...
{
	if (!HAIR_COLOR_ENABLE || !IsPC() || GetPart(CRaceData::PART_HAIR) == eHair) {
		return;
	}
...
void CInstanceBase::SetAcce(DWORD dwAcce, bool wikiPreview)
{
	...
	uint32_t realsash = dwAcce + 85000;
	if (realsash > 86000)
	{
		realsash -= 1000;
	}

	if (m_awPart[CRaceData::PART_ACCE] == realsash)
	{
		return;
	}
	...
	m_GraphicThingInstance.AttachAcce(pItemData, 100.0f/*fSpecular*/);
...
#if defined(ENABLE_SHINING_SYSTEM)
	__GetShiningEffect(pItemData, bEffect);
#endif
}
...
#if defined(ENABLE_ACCE_SYSTEM) && !defined(ENABLE_SHINING_SYSTEM)
...
/*
void CInstanceBase::SetParts(const uint32_t * c_pParts)
{
	if (IsPoly())
		return;
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
	switch (pItem->GetType())
	{
		case CItemData::ITEM_TYPE_WEAPON: {
			...
			uint32_t refine = max(pItem->GetRefine() + pItem->GetSocketCount(), 3) - 3;
#else
			uint32_t refine = max(pItem->GetRefine() + pItem->GetSocketCount(), CItemData::ITEM_SOCKET_MAX_NUM) - CItemData::ITEM_SOCKET_MAX_NUM;
#endif
...
*/
		...
		uint32_t itemVnum = pItem->GetIndex();
		...
		if (itemVnum == 49047 ||
			...
			for (int i = 0; i < CItemData::ITEM_SHINING_MAX_COUNT; i++) {
				if (strcmp(shiningTable.szShinings[i], "")) {
					if (i == 1) {
						__AttachArmorShiningEffect(i, shiningTable.szShinings[i], "snowflake_effect");
					} else if (i == 2) {
						__AttachArmorShiningEffect(i, shiningTable.szShinings[i], "shoulder_effect_left");
					} else {
...
#if defined(ENABLE_ACCE_SYSTEM) || defined(ENABLE_STOLE_COSTUME)
	if (
#if defined(ENABLE_ACCE_SYSTEM)
	(pItem->GetType() == CItemData::ITEM_TYPE_COSTUME && pItem->GetSubType() == CItemData::COSTUME_ACCE)
#if defined(ENABLE_STOLE_COSTUME)
 || 
#endif
#endif
#if defined(ENABLE_STOLE_COSTUME)
	(pItem->GetType() == CItemData::ITEM_TYPE_COSTUME && pItem->GetSubType() == CItemData::COSTUME_STOLE)
#endif
	) {
#if defined(ENABLE_SHINING_SYSTEM)
		__ClearAcceShiningEffect();
		...
		if (itemVnum == 85140 ||
			...
			if (bAdded == false && bAcceEffect == true) {
				__AttachAcceShiningEffect(0, "d:/ymir work/pc/common/effect/armor/acc_01.mse", "Bip01");
			}
		}
	}
#endif

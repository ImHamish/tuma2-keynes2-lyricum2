...
class CPythonCharacterManager : public CSingleton<CPythonCharacterManager>, public IAbstractCharacterManager, public IObjectManager
{
	...
	public:
		...
		void GetMobWithVnum(uint32_t, std::vector<CInstanceBase*>&);
#endif
	...
	public:
        ...
        protected:
            ...
            bool            m_abAutoHuntType[AUTO_ONOFF_MAX];
            TPixelPosition  m_rkLastPixelPos;

        public:
            bool            GetAutoOnOff();
            void            SetAutoOnOff(const bool b, const bool c = false);

            bool            GetAutoSystemByType(const uint8_t t);
            void            SetAutoSystemByType(const uint8_t t, const bool v);

            TPixelPosition  GetLastPixelPosition();
            void            SetLastPixelPosition(const TPixelPosition rkInstPos);
#endif
};

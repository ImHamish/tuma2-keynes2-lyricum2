...
#if defined(ENABLE_CHANGELOOK)
		if (CPythonChangeLook::Instance().IsCleanScroll(iItemIndex)) {
			return Py_BuildValue("i", TRUE);
		}
#endif
	}
...
{
	uint32_t itemVnum;
	if (!PyTuple_GetUnsignedInteger(poArgs, 0, &itemVnum))
	{
		return Py_BadArgument();
	}
	...
	if (itemVnum == 0)
	{
		return Py_BuildException("Invalid itemVnum: 0.");
	}

	CItemManager::Instance().SelectItemData(itemVnum);
...
#if defined(ENABLE_CHANGELOOK)
PyObject* itemIsPossibleChangeLook(PyObject* poSelf, PyObject* poArgs) {
	int32_t iItemIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iItemIndex)) {
		return Py_BadArgument();
	}
	...
	switch (pItemData->GetType()) {
		case CItemData::ITEM_TYPE_ARMOR: {
			bPossible = pItemData->GetSubType() == CItemData::ARMOR_BODY ? true : false;

			break;
		}
		case CItemData::ITEM_TYPE_WEAPON: {
			uint8_t itemSubType = pItemData->GetSubType();
			bPossible = itemSubType == CItemData::WEAPON_SWORD
						 || itemSubType == CItemData::WEAPON_DAGGER
						 || itemSubType == CItemData::WEAPON_BOW
						 || itemSubType == CItemData::WEAPON_TWO_HANDED
						 || itemSubType == CItemData::WEAPON_BELL
						 || itemSubType == CItemData::WEAPON_FAN
			? true : false;
		...
		case CItemData::ITEM_TYPE_COSTUME: {
			uint8_t itemSubType = pItemData->GetSubType();
			bPossible = itemSubType == CItemData::COSTUME_BODY
						 || itemSubType == CItemData::COSTUME_HAIR
#if defined(ENABLE_WEAPON_COSTUME_SYSTEM)
						 || itemSubType == CItemData::COSTUME_WEAPON
#endif
			? true : false;
		...
		default: {
			bPossible = CPythonChangeLook::instance().IsTransmutationTicket(iItemIndex) ? true : false;
			break;
		}
	}

	return Py_BuildValue("i", bPossible);
}
#endif
...
PyObject* itemGetAttr67MaterialVnum(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("i", pItemData->GetAttr67MaterialVnum());
}
#endif

#ifdef USE_MULTIPLE_OPENING
PyObject* itemCanBeMultipleOpened(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t vnum;
    ...
    const auto type = CItemManager::Instance().GetItemType(vnum);
    if (type == -1)
    {
        return Py_BuildException("%u item doesn't exists!", vnum);
    }
    ...
    if (ret == 0 && std::count(std::begin(ar_sAllowedMultipleOpeningItemVnums), std::end(ar_sAllowedMultipleOpeningItemVnums), vnum) > 0)
    {
        ret = 1;
    }
...
void initItem()
{
	...
	{
#if defined(ENABLE_CHANGELOOK)
		{"IsPossibleChangeLook", itemIsPossibleChangeLook, METH_VARARGS},
#endif
...
#endif
#ifdef USE_MULTIPLE_OPENING
        { "CanBeMultipleOpened", itemCanBeMultipleOpened, METH_VARARGS },
#endif
		{ NULL,								NULL,									NULL		 },
	};
	...
	PyModule_AddIntConstant(poModule, "APPLY_PC_UNUSED01",	CItemData::APPLY_PC_UNUSED01);
	PyModule_AddIntConstant(poModule, "APPLY_PC_UNUSED02",	CItemData::APPLY_PC_UNUSED02);
...
#endif
#if defined(ENABLE_APPLY_MAGICRES_REDUCTION)
	PyModule_AddIntConstant(poModule, "POINT_RESIST_MAGIC_REDUCTION", POINT_RESIST_MAGIC_REDUCTION);
#endif
	PyModule_AddIntConstant(poModule, "APPLY_ATT_RACES", CItemData::APPLY_ATT_RACES);
	PyModule_AddIntConstant(poModule, "POINT_ATT_RACES", POINT_ATT_RACES);
	PyModule_AddIntConstant(poModule, "APPLY_RES_RACES", CItemData::APPLY_RES_RACES);
	PyModule_AddIntConstant(poModule, "POINT_RES_RACES", POINT_RES_RACES);
#if defined(ENABLE_PETSKIN)
	PyModule_AddIntConstant(poModule, "APPLY_PETSKIN", CItemData::APPLY_PETSKIN);
#endif
#if defined(ENABLE_MOUNTSKIN)
	PyModule_AddIntConstant(poModule, "APPLY_MOUNTSKIN", CItemData::APPLY_MOUNTSKIN);
#endif
#if defined(ENABLE_HALLOWEEN_EVENT_2022) && !defined(USE_NO_HALLOWEEN_EVENT_2022_BONUSES)
    PyModule_AddIntConstant(poModule, "APPLY_FEAR", CItemData::APPLY_FEAR);
    PyModule_AddIntConstant(poModule, "APPLY_RESISTANCE_FEAR", CItemData::APPLY_RESISTANCE_FEAR);
    PyModule_AddIntConstant(poModule, "POINT_FEAR", POINT_FEAR);
    PyModule_AddIntConstant(poModule, "POINT_RESISTANCE_FEAR", POINT_RESISTANCE_FEAR);
#endif
#ifdef ENABLE_SEALBIND_SYSTEM
...
#if defined(ENABLE_PETSKIN)
	PyModule_AddIntConstant(poModule, "COSTUME_PETSKIN", CItemData::COSTUME_PETSKIN);
#endif
#if defined(ENABLE_MOUNTSKIN)
	PyModule_AddIntConstant(poModule, "COSTUME_MOUNTSKIN", CItemData::COSTUME_MOUNTSKIN);
#endif
...
#endif
	PyModule_AddIntConstant(poModule, "ITEM_GACHA", CItemData::ITEM_GACHA);
	...
	PyModule_AddIntConstant(poModule, "APPLY_ANTI_EXP", CItemData::APPLY_ANTI_EXP);
#endif

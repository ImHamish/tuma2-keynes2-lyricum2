...
bool CPythonMiniMap::Create()
{
	...
	const std::string strBossMarkAlive = strImageRoot + "minimap/boss_mark_alive.tga";
	const std::string strBossMarkDead = strImageRoot + "minimap/boss_mark_dead.tga";
	...
	pSubImage = (CGraphicSubImage*)CResourceManager::Instance().GetResourcePointer(strBossMarkAlive.c_str());
	m_BossMarkAlive.SetImagePointer(pSubImage);

	pSubImage = (CGraphicSubImage*)CResourceManager::Instance().GetResourcePointer(strBossMarkDead.c_str());
	m_BossMarkDead.SetImagePointer(pSubImage);
...
void CPythonMiniMap::RenderAtlas(float fScreenX, float fScreenY)
{
	...
	for (const auto& v : m_AtlasBossMark) {
		m_BossMarkAlive.SetPosition(v.m_fScreenX, v.m_fScreenY);
		m_BossMarkDead.SetPosition(v.m_fScreenX, v.m_fScreenY);

		if (static_cast<int>(timeGetTime() / 500) % 2 && v.is_alive)
			continue;

		(v.is_alive ? m_BossMarkAlive : m_BossMarkDead).Render();
	}
...
void CPythonMiniMap::ClearBossMarkInfo()
{
	m_AtlasBossMark.clear();
}

void CPythonMiniMap::RegisterBossAtlasInfo(const uint32_t vnum, const int32_t x, const int32_t y, const uint32_t time, const uint32_t real_time, const bool is_alive)
{
	SBossPositionInfo info{};
	info.m_fX = static_cast<float>(x);
	info.m_fY = static_cast<float>(y);
	info.m_vnum = vnum;
	info.respawn_time = time;
	info.is_alive = is_alive;
	info.real_time = real_time;
	info.m_fScreenX = info.m_fX / m_fAtlasMaxX * m_fAtlasImageSizeX - static_cast<float>(m_BossMarkAlive.GetWidth()) / 2.0f;
	info.m_fScreenY = info.m_fY / m_fAtlasMaxY * m_fAtlasImageSizeY - static_cast<float>(m_BossMarkAlive.GetHeight()) / 2.0f;
	m_AtlasBossMark.emplace_back(info);
}

bool CPythonMiniMap::GetBossAtlasInfo(const float x, const float y, uint32_t* vnum, uint32_t* respawn_time, uint32_t* real_time, bool* is_alive)
{
	const float fRealX = (x - m_fAtlasScreenX) * (m_fAtlasMaxX / m_fAtlasImageSizeX);
	const float fRealY = (y - m_fAtlasScreenY) * (m_fAtlasMaxY / m_fAtlasImageSizeY);
	const float fCheckWidth = (m_fAtlasMaxX / m_fAtlasImageSizeX) * static_cast<float>(m_BossMarkAlive.GetWidth()) / 2.0f;
	const float fCheckHeight = (m_fAtlasMaxY / m_fAtlasImageSizeY) * static_cast<float>(m_BossMarkAlive.GetHeight()) / 2.0f;

	for (const auto& v : m_AtlasBossMark) {
		if (v.m_fX - fCheckWidth < fRealX && v.m_fX + fCheckWidth > fRealX && v.m_fY - fCheckWidth < fRealY && v.m_fY + fCheckHeight > fRealY) {
			*vnum = v.m_vnum;
...
void CPythonMiniMap::Destroy()
{
	...
	m_BossMarkAlive.Destroy();
	m_BossMarkDead.Destroy();

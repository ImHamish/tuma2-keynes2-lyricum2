...
#ifdef ATTR_LOCK
void CPythonExchange::SetItemAttrLocked(int iPos, short dwIndex, bool bSelf)
{
	if (iPos >= EXCHANGE_ITEM_MAX_NUM)
		return;

	if (bSelf)
		m_self.lockedattr[iPos] = dwIndex;
	else
		m_victim.lockedattr[iPos] = dwIndex;
}

short CPythonExchange::GetItemAttrLocked(int iPos, bool bSelf)
{
	if (iPos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;

	short sLockedAttr = bSelf == true ? m_self.lockedattr[iPos] : m_victim.lockedattr[iPos];
	return sLockedAttr;
}
#endif
...
uint64_t CPythonExchange::GetElkFromTarget() const
{
	return m_victim.elk;
}

uint64_t CPythonExchange::GetElkFromSelf() const
{
	return m_self.elk;
}
#else
...
void CPythonExchange::SetItemTransmutation(int32_t iPos, uint32_t dwTransmutation, bool bSelf) {
	if (iPos < 0 || iPos >= EXCHANGE_ITEM_MAX_NUM) {
		return;
	}
...
uint32_t CPythonExchange::GetItemTransmutation(int32_t iPos, bool bSelf) {
	if (iPos < 0 || iPos >= EXCHANGE_ITEM_MAX_NUM) {
		return 0;
	}

	return bSelf == true ? m_self.transmutation[iPos] : m_victim.transmutation[iPos];
}
#endif

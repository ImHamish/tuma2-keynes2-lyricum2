...
#endif
{
	...
	granny_controlled_animation_builder* pBuilder = GrannyBeginControlledAnimation(StartTime, Animation);
	...
	if (pBuilder) {
		...
		if (GrannyFindTrackGroupForModel(Animation, GrannyGetSourceModel(InstanceOfModel)->Name, &TrackGroupIndex)) {
			GrannySetTrackGroupLOD(pBuilder, TrackGroupIndex, true, 1.0f);
			GrannySetTrackGroupTarget(pBuilder, TrackGroupIndex, InstanceOfModel);
		} else {
			GrannySetTrackGroupLOD(pBuilder, 1.0f, true, 1.0f);
			GrannySetTrackGroupTarget(pBuilder, 0, InstanceOfModel);
		}

		m_pgrnCtrl = GrannyEndControlledAnimation(pBuilder);
	}
#else

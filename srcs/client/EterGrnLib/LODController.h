...
#if defined(ENABLE_GRANNY_INBUILD_ANIMATION)
		void SetMotionPointer(const std::shared_ptr<CGrannyMotion>, float, int32_t, float);
		void ChangeMotionPointer(const std::shared_ptr<CGrannyMotion>, int32_t, float);
#else
		void SetMotionPointer(const CGrannyMotion*, float, int32_t, float);
		void ChangeMotionPointer(const CGrannyMotion*, int32_t, float);
#endif
		void	SetMotionAtEnd();

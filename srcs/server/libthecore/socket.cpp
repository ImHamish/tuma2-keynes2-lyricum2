...
int32_t socket_udp_read(socket_t desc, char* read_point, size_t space_left, struct sockaddr* from, socklen_t* fromlen)
{
	return recvfrom(desc, read_point, space_left, 0, from, fromlen);
}

int32_t socket_read(socket_t desc, char* read_point, size_t space_left)
{
	const auto ret = recv(desc, read_point, space_left, 0);
...
#ifdef _WIN32
	const int32_t wsa_error = WSAGetLastError();
...
int32_t socket_write_tcp(socket_t desc, const char* txt, int32_t length)
{
	const int32_t bytes_written = send(desc, txt, length, 0);
...
int32_t socket_write(socket_t desc, const char* data, size_t length)
{
...
socket_t socket_bind(const char* ip, int32_t port, int32_t protocol)
{
	socket_t s;
#ifdef _WIN32
	SOCKADDR_IN sa;
	...
	if ((s = socket(AF_INET, protocol, 0)) < 0)
	{
		...
		sys_err("socket_bind(%s, %u): WSAGetLastError returned %d", ip, port, wsa_error);
#endif
...
socket_t socket_tcp_bind(const char* ip, int32_t port)
{
	return socket_bind(ip, port, SOCK_STREAM);
}

socket_t socket_udp_bind(const char* ip, int32_t port)
{
...
socket_t socket_accept(socket_t s, struct sockaddr_in* peer)
{
	...
	if ((desc = accept(s, reinterpret_cast<sockaddr*>(peer), &i)) == -1)
	{
...
socket_t socket_connect(const char* host, uint16_t port)
{
	...
	if ((rslt = connect(s, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr))) < 0)
	{
		socket_close(s);
...
void socket_dontroute(socket_t s)
{
	...
	if (setsockopt(s, SOL_SOCKET, SO_DONTROUTE, reinterpret_cast<const char*>(&set), sizeof(int32_t)) < 0)
	{
...
void socket_lingeroff(socket_t s)
{
    ...
    linger.l_linger	= 0;
#endif
	if (setsockopt(s, SOL_SOCKET, SO_LINGER, reinterpret_cast<const char*>(&linger), sizeof(linger)) < 0)
	{
...
void socket_rcvbuf(socket_t s, uint32_t opt)
{
	socklen_t optlen;
...
void socket_sndbuf(socket_t s, uint32_t opt)
{
	socklen_t optlen;

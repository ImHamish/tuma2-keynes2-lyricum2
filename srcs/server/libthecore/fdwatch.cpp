...
LPFDWATCH fdwatch_new(int32_t nfiles)
{
    LPFDWATCH fdw;
    int32_t kq;
    ...
    CREATE(fdw->fd_event_idx, int32_t, nfiles);
    CREATE(fdw->fd_rw, int32_t, nfiles);
...
int32_t fdwatch(LPFDWATCH fdw, struct timeval *timeout)
{
    int32_t	i, r;
    struct timespec ts;
    ...
    memset(fdw->fd_event_idx, 0, sizeof(int32_t) * fdw->nfiles);

    for (i = 0; i < r; i++)
    {
	int32_t fd = fdw->kqrevents[i].ident;
...
void fdwatch_register(LPFDWATCH fdw, int32_t flag, int32_t fd, int32_t rw)
{
    if (flag == EV_DELETE)
    {
...
void fdwatch_add_fd(LPFDWATCH fdw, socket_t fd, void * client_data, int32_t rw, int32_t oneshot)
{
	int32_t flag;
...
void fdwatch_clear_event(LPFDWATCH fdw, socket_t fd, uint32_t event_idx)
{
    assert(event_idx < (uint32_t)fdw->nfiles * 2);
...
int32_t fdwatch_check_event(LPFDWATCH fdw, socket_t fd, uint32_t event_idx)
{
    assert(event_idx < (uint32_t)fdw->nfiles * 2);
...
int32_t fdwatch_get_ident(LPFDWATCH fdw, uint32_t event_idx)
{
    assert(event_idx < (uint32_t)fdw->nfiles * 2);
    return fdw->kqrevents[event_idx].ident;
}

int32_t fdwatch_get_buffer_size(LPFDWATCH fdw, socket_t fd)
{
    int32_t event_idx = fdw->fd_event_idx[fd];
...
void * fdwatch_get_client_data(LPFDWATCH fdw, uint32_t event_idx)
{
    int32_t fd;
...
#ifdef _WIN32
static int32_t win32_init_refcount = 0;
...
LPFDWATCH fdwatch_new(int32_t nfiles)
{
	...
	CREATE(fdw->select_rfdidx, int32_t, nfiles);
...
static int32_t fdwatch_get_fdidx(LPFDWATCH fdw, socket_t fd)
{
	int32_t i;
...
void fdwatch_add_fd(LPFDWATCH fdw, socket_t fd, void* client_data, int32_t rw, int32_t oneshot)
{
	int32_t idx = fdwatch_get_fdidx(fdw, fd);
	if (idx < 0)
	{
...
int32_t fdwatch_check_fd(LPFDWATCH fdw, socket_t fd)
{
	int32_t idx = fdwatch_get_fdidx(fdw, fd);
	if (idx < 0)
	{
...
void* fdwatch_get_client_data(LPFDWATCH fdw, uint32_t event_idx)
{
	int32_t idx = fdw->select_rfdidx[event_idx];
...
int32_t fdwatch_get_ident(LPFDWATCH fdw, uint32_t event_idx)
{
	...
	return (int32_t)fdw->select_fds[idx];
}
...
int32_t fdwatch_check_event(LPFDWATCH fdw, socket_t fd, uint32_t event_idx)
{
	...
	int32_t result = fdwatch_check_fd(fdw, fd);
	if (result & FDW_READ)
	{
		return FDW_READ;
	}

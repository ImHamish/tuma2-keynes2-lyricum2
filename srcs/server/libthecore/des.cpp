...
extern uint32_t SP_boxes[8][64];
//extern uint32_t KeyPerm[8][64];
 ...
 */
#define BYTES_TO_DWORD(b,d) (d  = ((uint32_t)(*((b)++))), \
                             d |= ((uint32_t)(*((b)++)))<< 8, \
                             d |= ((uint32_t)(*((b)++)))<<16, \
                             d |= ((uint32_t)(*((b)++)))<<24)

#define DWORD_TO_4BYTES(d,b) (*((b)++)=(uint8_t)(((d)   )&0xff), \
                              *((b)++)=(uint8_t)(((d)>> 8)&0xff), \
                              *((b)++)=(uint8_t)(((d)>>16)&0xff), \
                              *((b)++)=(uint8_t)(((d)>>24)&0xff))
...
void DES_ECB_mode(uint8_t* Input, /* 8 bytes of input data              */
                  uint8_t* Output, /* 8 bytes of output data             */
                  const uint32_t* KeySchedule, /* [16][2] array of DWORDs            */
                  uint8_t Operation) /* DES_ECB_ENCRYPT or DES_ECB_DECRYPT */
{
	static uint8_t *bInp, *bOut;
	static uint32_t dwLeft, dwRigh, dwTmp, dwTmp1;
...
int32_t DES_Encrypt(uint32_t* DstBuffer, const uint32_t* SrcBuffer, const uint32_t* KeyAddress, uint32_t Length, uint32_t* IVector)
{
	uint32_t i;
	uint32_t buffer[2];
	...
	for (i = 0; i < (Length >> 2); i = i + 2)
	{
		...
		DES_ECB_mode((uint8_t*)buffer, (uint8_t*)buffer, KeyAddress, DES_ECB_ENCRYPT);
...
int32_t DES_Decrypt(uint32_t* DstBuffer, const uint32_t* SrcBuffer, const uint32_t* KeyAddress, uint32_t Length, uint32_t* IVector)
{
	uint32_t i;
	uint32_t buffer[2], ivectorL, ivectorR, oldSrcL, oldSrcR;
	...
	for (i = 0; i < (Length >> 2); i = i + 2)
	{
		...
		DES_ECB_mode((uint8_t*)buffer, (uint8_t*)buffer, KeyAddress, DES_ECB_DECRYPT);

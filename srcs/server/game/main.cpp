	...
	if (!(pulse % ht->passes_per_sec))
	{
...

		{
			...
			if (!g_bAuthServer)
			{
				...
				regen_spawn_static(time_info->tm_hour, time_info->tm_min, time_info->tm_sec);
			}
		}
	}
...
int start(int argc, char **argv)
{
	...
	main_fdw = fdwatch_new(MAX_ALLOW_USER);
	...
	if (!g_bAuthServer)
	{
		db_clientdesc->UpdateChannelStatus(get_dword_time(), true);
	}
	...
	for (event_idx = 0; event_idx < num_events; ++event_idx)
	{
		...
		if (!d)
		{
			...
			else
			{
				if (!g_bAuthServer)
				{
					if (FDW_READ == fdwatch_check_event(fdw, p2p_socket, event_idx))
					{

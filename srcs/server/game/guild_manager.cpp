...
{
	return quest::CQuestManager::instance().GetEventFlag("guild_disband_delay") * (test_server ? 60 : 3600);
}

int CGuildManager::GetWithdrawDelay()
{
	return quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay") * (test_server ? 60 : 3600);
}
...
{
	const LPDESC pKillerDesc = killer->GetDesc();
	const LPDESC pVictimDesc = victim->GetDesc();
	if (!pKillerDesc || !pVictimDesc) {
		return;
	}

	if ((strcmp(killer->GetDesc()->GetHostName(), victim->GetDesc()->GetHostName()) == 0)) {
		return;
	}

...
void DBManager::SetPremiumByType(LPDESC pDesc, uint8_t type, int32_t time)
{
    if (!pDesc || type >= PREMIUM_MAX_NUM)
    {
        return;
    }

    const TAccountTable& r = pDesc->GetAccountTable();

    TPacketGDPremiumUpdate p;
    trim_and_lower(r.login, p.login, sizeof(p.login));
    p.type = type;
    p.time = time;

    db_clientdesc->DBPacket(HEADER_GD_UPDATE_PREMIUM, pDesc->GetHandle(), &p, sizeof(TPacketGDPremiumUpdate));
}
#endif
		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
...

					{
                        ...
                        str_to_number(aiPremiumTimes[PREMIUM_BONUS1], row[col++]);
                        str_to_number(aiPremiumTimes[PREMIUM_BONUS2], row[col++]);
                        str_to_number(aiPremiumTimes[PREMIUM_BONUS3], row[col++]);
#endif
#ifdef USE_AUTO_HUNT
					...
					else
					{
						...
						DESC_MANAGER::instance().LoginSuccessByIP(d->GetHostName());
#endif
...
#else
void DBManager::SendMoneyLog(uint8_t type, uint32_t vnum, uint32_t gold)
#endif
{
	if (gold == 0)
	{

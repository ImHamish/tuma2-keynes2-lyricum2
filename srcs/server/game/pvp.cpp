...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkVictim->IsGM() && pkVictim->IsAffectFlag(AFF_INVISIBILITY))
		return false;
	...
	{
		static std::set<uint32_t> maps = {
			151,210,62,68,301,302,303
		};

		if (maps.find(pkVictim->GetMapIndex()) != maps.end())
		{
			return false;
		}
	}

...
{
	if (fForce || t > m_tLastChannelStatusUpdateTime)
	{
		m_tLastChannelStatusUpdateTime = get_dword_time() + (5 * 60 * 1000);
		...
		channelStatus.port = mother_port;
		...
		{
			channelStatus.status = iTotal > g_iFullUserCount ? 3 : iTotal > g_iBusyUserCount ? 2 : 1;
		}

		DBPacket(HEADER_GD_UPDATE_CHANNELSTATUS, 0, &channelStatus, sizeof(TChannelStatus));
	}
}

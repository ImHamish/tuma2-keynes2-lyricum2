...
/*
	DBManager::instance().FuncQuery([this](SQLMsg* msg) { __P2PUpdateGrade(msg); },
		"SELECT grade, name, auth+0 FROM guild_grade%s WHERE guild_id = %u and grade = %d", get_table_postfix(), m_data.guild_id, grade);
*/
}
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	if (ch->GetPoint(POINT_ANTI_EXP) != 0)
	{
		return false;
	}
#endif
...
void CGuild::AddComment(LPCHARACTER ch, const std::string& str) {
	...
	if (m_guildPostCommentPulse > thecore_pulse() && !ch->IsGM())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "You can post a new comment in %02d minutes and %02d seconds!", iMin, iSec);
		return;
	}
...
, m_name(name)
		...
		void operator()(LPCHARACTER ch) {
#if defined(ENABLE_MESSENGER_BLOCK)
			if (MessengerManager::instance().CheckMessengerList(ch->GetName(), m_name, SYST_BLOCK)) {
				return;
			}
#endif
...
#if defined(ENABLE_MESSENGER_BLOCK)
	strlcpy(p2.name, name, sizeof(p2.name));
#endif
...
CGuild::GuildJoinErrCode CGuild::VerifyGuildJoinableCondition( const LPCHARACTER pchInvitee )
{
	...
	else if (g_bGuildInviteLimit)
	{
		...
		if ( pMsg->Get()->uiNumRows > 0 )
		{
			...
			{
				limit_time += quest::CQuestManager::instance().GetEventFlag("guild_invite_limit") * 3600;
			}

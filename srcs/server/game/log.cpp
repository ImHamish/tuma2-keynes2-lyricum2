...
void LogManager::MoneyLog(BYTE type, DWORD vnum, uint64_t gold)
{
...
#else
void LogManager::MoneyLog(BYTE type, DWORD vnum, uint32_t)
{
	...
	Query("INSERT DELAYED INTO money_log%s VALUES (NOW(), %d, %d, %lu)", get_table_postfix(), type, vnum, gold);
}
#endif
...
void LogManager::HackReport(uint32_t pid, const char* name, const char* ip, const char* reason) {
	Query("INSERT INTO `hack_report` (`pid`, `name`, `ip`, `reason`, `when`) VALUES(%u, '%s', '%s', '%s', NOW())", pid, name, ip, reason);
}
#endif

#if defined(ENABLE_OKEY_CARD_GAME)
void LogManager::OkayEventLog(uint32_t pid, const char* szName, uint32_t points)
{
	Query("INSERT INTO `okay_event` (`pid`, `name`, `points`) VALUES(%u, '%s', %u)",
			pid, szName, points);
}
#endif

...
uint64_t CShopEx::Buy(LPCHARACTER ch, BYTE pos)
#else
uint32_t CShopEx::Buy(LPCHARACTER ch, BYTE pos)
#endif
{
...
#if defined(ENABLE_NEW_GOLD_LIMIT)
		if (ch->GetGold() < static_cast<uint64_t>(dwPrice))
		{
			sys_log(1, "ShopEx::Buy : Not enough money : %s has %llu, price %llu", ch->GetName(), ch->GetGold(), static_cast<uint64_t>(dwPrice));
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
		}
#else
		if (ch->GetGold() < static_cast<uint32_t>(dwPrice))
		{
			sys_log(1, "ShopEx::Buy : Not enough money : %s has %lu, price %lu", ch->GetName(), ch->GetGold(), static_cast<uint32_t>(dwPrice));
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
		}
#endif
		break;

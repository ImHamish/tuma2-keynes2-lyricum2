...
, const char*
		...
		// War state relative
		void		NotifyGuildMaster(BYTE type, DWORD idx, const char * format, ...);
		...
		void ChangeGuildSymbol(LPCHARACTER ch);
		static bool SendGuildSymbolAuthData(LPDESC desc, uint8_t* key);
		void ClearSymbolAuthKey(DWORD pid);
	...
	private:
		...
		// END_OF_GUILD_MEMBER_COUNT_BONUS
		int32_t m_iLastChangeSymbolPulse;

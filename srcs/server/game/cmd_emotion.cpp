...
#if defined(ENABLE_TEXTS_RENEWAL)
		ch->ChatPacketNew(CHAT_TYPE_INFO, 1429, "%s", tch->GetName());
#else
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't perform this action because %s blocked you.", tch->GetName());
#endif
		return;
	}
#endif
...
ACMD(do_emotion)
{
	...
	if (bMoving || bAttack)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Trebuie sa stai pe loc pentru a utiliza emotia.");
		return;
	}

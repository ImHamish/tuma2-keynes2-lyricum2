...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		if (chA != NULL)
		{
			...
			chA->ChatPacketNew(CHAT_TYPE_INFO, 299, "");
#endif
		...
		{
#if defined(ENABLE_TEXTS_RENEWAL)
			chB->ChatPacketNew(CHAT_TYPE_INFO, 299, "");
#endif
	...
	switch (info->state)
	{
		case 0:
			{
				...
				{
#if defined(ENABLE_TEXTS_RENEWAL)
					chA->ChatPacketNew(CHAT_TYPE_INFO, 348, "");
					chB->ChatPacketNew(CHAT_TYPE_INFO, 348, "");
#endif
				}
				else
				{
...
#if defined(ENABLE_TEXTS_RENEWAL)
					chA->ChatPacketNew(CHAT_TYPE_INFO, 349, "%d", count);
					chB->ChatPacketNew(CHAT_TYPE_INFO, 349, "%d", count);
#endif
				}
				
#if defined(ENABLE_TEXTS_RENEWAL)
				chA->ChatPacketNew(CHAT_TYPE_INFO, 222, "");
				chB->ChatPacketNew(CHAT_TYPE_INFO, 222, "");
		...
		case 1:
			{
#if defined(ENABLE_TEXTS_RENEWAL)
				chA->ChatPacketNew(CHAT_TYPE_INFO, 301, "");
				chB->ChatPacketNew(CHAT_TYPE_INFO, 301, "");
...
EVENTFUNC(duel_time_out)
{
	...
	else
	{
		switch (info->state)
		{
			case 0:
#if defined(ENABLE_TEXTS_RENEWAL)
				chA->ChatPacketNew(CHAT_TYPE_INFO, 224, "");
				chB->ChatPacketNew(CHAT_TYPE_INFO, 224, "");
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
#if defined(ENABLE_TEXTS_RENEWAL)
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 109, "%s", pCharA->GetName());
				pCharB->ChatPacketNew(CHAT_TYPE_INFO, 109, "%s", pCharA->GetName());
			...
			else
			{
				restart = true;
#if defined(ENABLE_TEXTS_RENEWAL)
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 110, "%s", pCharA->GetName());
				pCharB->ChatPacketNew(CHAT_TYPE_INFO, 110, "%s", pCharA->GetName());
				SendChatPacketToObserver(CHAT_TYPE_NOTICE, 110, "%s", pCharA->GetName());
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 709, "%s#%d#%s#%d", pCharA->GetName(), GetPlayerAPID(), pCharB->GetName(), GetPlayerBPID());
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 709, "%s#%d#%s#%d", pCharA->GetName(), GetPlayerAPID(), pCharB->GetName(), GetPlayerBPID());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
#if defined(ENABLE_TEXTS_RENEWAL)
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 109, "%s", pCharB->GetName());
				pCharB->ChatPacketNew(CHAT_TYPE_INFO, 109, "%s", pCharB->GetName());
			...
			else
			{
				restart = true;
#if defined(ENABLE_TEXTS_RENEWAL)
				pCharA->ChatPacketNew(CHAT_TYPE_INFO, 110, "%s", pCharB->GetName());
				pCharB->ChatPacketNew(CHAT_TYPE_INFO, 110, "%s", pCharB->GetName());
...
#if defined(ENABLE_TEXTS_RENEWAL)
		if (pCharA != NULL) {
			pCharA->ChatPacketNew(CHAT_TYPE_INFO, 223, "");
		}
		if (pCharB != NULL) {
			pCharB->ChatPacketNew(CHAT_TYPE_INFO, 223, "");
		}
...
#if defined(ENABLE_TEXTS_RENEWAL)
		if (pCharA != NULL) {
			pCharA->ChatPacketNew(CHAT_TYPE_INFO, 221, "");
		}
		
		if (pCharB != NULL) {
			pCharB->ChatPacketNew(CHAT_TYPE_INFO, 221, "");
		}
...
#if defined(ENABLE_TEXTS_RENEWAL)
		if (GetPlayerB() != NULL) {
			GetPlayerB()->ChatPacketNew(CHAT_TYPE_INFO, 232, "");
		}
#endif
...
#if defined(ENABLE_TEXTS_RENEWAL)
		if (GetPlayerA() != NULL) {
			GetPlayerA()->ChatPacketNew(CHAT_TYPE_INFO, 232, "");
		}
#endif
...
#if defined(ENABLE_TEXTS_RENEWAL)
void CArena::SendChatPacketToObserver(BYTE type, DWORD idx, const char * format, ...)
{
	char chatbuf[256];
	va_list args;
	...
	for (; iter != m_mapObserver.end(); iter++) {
		...
		if (pChar) {
			if (pChar->GetDesc() != NULL) {
				pChar->ChatPacketNew(type, idx, chatbuf);
			}
		}
	}
}

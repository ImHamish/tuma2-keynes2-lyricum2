...
WORD DSManager::GetBasePosition(const LPITEM pItem) const
{
	...
	return 300 + 25 + (col_type * DRAGON_SOUL_GRADE_MAX * DRAGON_SOUL_BOX_SIZE + row_type * DRAGON_SOUL_BOX_SIZE);
#else
	return 300 + 25 + (col_type * DRAGON_SOUL_STEP_MAX * DRAGON_SOUL_BOX_SIZE + row_type * DRAGON_SOUL_BOX_SIZE);
#endif
}
...
void DSManager::DoRefineAll(LPCHARACTER ch, uint8_t subheader, uint8_t type, uint8_t grade) {
	...
	WORD wBaseCell = 300 + 25 + (192 * type) + (grade * DRAGON_SOUL_BOX_SIZE);

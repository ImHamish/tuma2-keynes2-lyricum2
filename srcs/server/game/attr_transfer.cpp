...
bool AttrTransfer_make(LPCHARACTER ch)
{
	...
	for (auto i = 0; i < ITEM_ATTRIBUTE_NORM_END; ++i) {
		if (items[1]->GetAttributeType(i) > 0 && items[1]->GetAttributeValue(i) > 0) {
			hasAttr = true;
			break;
		}
	}
	...
	if (!hasAttr) {
#if defined(ENABLE_TEXTS_RENEWAL)
		ch->ChatPacketNew(CHAT_TYPE_INFO, 1480, "");
#else
		ch->ChatPacket(CHAT_TYPE_INFO, "The first costume must have at least one bonus.");
#endif
		return false;
	}

	for (auto i = 0; i < ITEM_ATTRIBUTE_NORM_END; ++i) {
		if (items[2]->GetAttributeType(i) > 0 && items[2]->GetAttributeValue(i) > 0) {
			hasAttr = true;
			break;
		}
	}
...
#if defined(ENABLE_ATTR_COSTUMES)
		if (i == 5 || i == 6) {
#if defined(ENABLE_STOLE_COSTUME)
			if (i == 5 && items[1]->GetSubType() == COSTUME_STOLE) {

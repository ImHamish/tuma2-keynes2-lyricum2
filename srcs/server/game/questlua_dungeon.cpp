...
namespace quest
{
	...
	int dungeon_join_coords(lua_State* L)
	{
		...
		if (!pDungeon) {
			sys_err("dungeon %d cannot be started.", mapidx);
			return 0;
		}
	...
	int dungeon_get_unique_vid(lua_State* L)
	{
		...
		lua_pushnumber(L, dungeon->GetUniqueVid(lua_tostring(L, 2)));
		return 1;
	}
	...
	int dungeon_purge_unique(lua_State* L)
	{
		...
		dungeon->PurgeUnique(lua_tostring(L, 2));
		return 0;
	}
	...
	int dungeon_set_flag(lua_State* L)
	{
		...
		{
			sys_err("not enough arguments (arg1: %d).", (int32_t)lua_tonumber(L, 1));
			return 0;
		}
		else if (!lua_isnumber(L, 3))
		{
			sys_err("not enough arguments (arg1: %d, arg2: %s).", (int32_t)lua_tonumber(L, 1), lua_tostring(L, 2));
			return 0;
		}
	...
	int dungeon_set_vid_invincible(lua_State* L)
	{
	...
	int dungeon_exit_all_lobby(lua_State* L)
	{
		if (!lua_isnumber(L, 1) 
			|| !lua_isnumber(L, 2) 
			|| !lua_isnumber(L, 3))
		...
		uint32_t mapidx = static_cast<uint32_t>(lua_tonumber(L, 1));

		LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(mapidx);
		if (!pDungeon)
		{
			sys_err("dungeon %u cannot be found.", mapidx);
			return 0;
		}

		pDungeon->ExitAllLobby(static_cast<uint32_t>(lua_tonumber(L, 2) * 100), static_cast<uint32_t>(lua_tonumber(L, 3) * 100));
		return 0;
	}
	...
	int dungeon_kill_all_monsters(lua_State* L)
	{
		//if (lua_gettop(L) < 1 || !lua_isnumber(L, 1))
		if (!lua_isnumber(L, 1))
		{
...
#ifdef __DEFENSE_WAVE__
	int dungeon_kill_all_monstershydra(lua_State* L)
	{
		//if (lua_gettop(L) < 1 || !lua_isnumber(L, 1))
		if (!lua_isnumber(L, 1))
		{
	...
	int dungeon_restore_mast_partial_life(lua_State* L)
	{
		//if (lua_gettop(L) < 1 || !lua_isnumber(L, 1))
		if (!lua_isnumber(L, 1))
		{
	...
	int dungeon_check_entrance(lua_State* L)
	{
	...
	int dungeon_remove_item(lua_State* L)
	{
		CQuestManager & q = CQuestManager::instance();
		std::string r = "";
		...
		if (party)
		{
			...
			for (auto it = f.vecPIDs.begin(); it != f.vecPIDs.end(); it++)
			{
				...
				if (tch && tch->IsPC())
				{
					...
					tch->UpdateExtBattlePassMissionProgress(COMPLETE_DUNGEON, 1, (DWORD)(tch->GetMapIndex() / 1000));
#endif
...
#if defined(ENABLE_HALLOWEEN_EVENT_2022)
					uint32_t idxHalloweenMaps[] = { 231, 232, 233 };
					if (time < 1800 && std::count(std::begin(idxHalloweenMaps), std::end(idxHalloweenMaps), idx) > 0) {
						cooldown /= 2;
					}
...
#if defined(USE_BATTLEPASS)
			ch->UpdateExtBattlePassMissionProgress(COMPLETE_DUNGEON, 1, (DWORD)(ch->GetMapIndex() / 1000));
#endif
	...
	int dungeon_check_entrance_meley(lua_State* L)
	{
		std::string r = "";
	...
	int dungeon_remove_item_meley(lua_State* L)
	{
		CQuestManager & q = CQuestManager::instance();
		std::string r = "";
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isstring(L, 3))
	...
	int dungeon_attack_meley(lua_State* L)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("not enough arguments.");
	...
	int dungeon_set_meley_last_statue(lua_State* L)
	{
	...
	int dungeon_kill_meley(lua_State* L)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("not enough arguments.");
	...
	int dungeon_battlepass_update_progress(lua_State* L)
	{
		...
		f.dungeon_index = mapidx;

		dungeon->ForEachMember(f);
		return 0;
	}
#endif
	...
	int dungeon_unique_set_maxhp(lua_State* L)
	{
		...
		dungeon->UniqueSetMaxHP(lua_tostring(L, 2), (int32_t)lua_tonumber(L, 3));
		lua_pushnumber(L, 1);
		return 1;
	}
	...
	int dungeon_unique_get_hp_perc(lua_State* L)
	{
		...
		lua_pushnumber(L, dungeon->GetUniqueHpPerc(lua_tostring(L, 2)));
		return 1;
	}
	...
	struct FUpdateTrackWindow {
		...
		void operator () (LPENTITY ent) {
			if (ent->IsType(ENTITY_CHARACTER) == true) {
				...
				if (pChar && pChar->IsPC()) {
					if (pChar->GetProtectTime("track_dungeon")) {
						pChar->GetDungeonCooldown(bossVnum);
					}
				}
			}
		}
	...
	int dungeon_track_update(lua_State* L)
	{
		...
		int32_t mapidx = (int32_t)lua_tonumber(L, 1);
		const LPDUNGEON dungeon = CDungeonManager::instance().FindByMapIndex(mapidx);
		...
		const LPSECTREE_MAP pSectree = SECTREE_MANAGER::instance().GetMap((uint32_t)lua_tonumber(L, 2));
		if (pSectree != NULL) {
			FUpdateTrackWindow func((uint32_t)lua_tonumber(L, 3));

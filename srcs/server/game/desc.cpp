...
void DESC::Initialize()
{
	...
	m_bAdminMode = false;
	m_bPong = true;
	m_chLastStatusReqTime = get_global_time();
...
void DESC::Packet(const void * c_pvData, int iSize)
{
	...
	if (m_bPhase == PHASE_CLOSE) // 끊는 상태면 보내지 않는다.
		return;
		...
		else
		{
			if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
			{
				...
				if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
				{
					...
					sys_err("buffer mem_size overflow : memsize(%u) write_pos(%u) iSize(%d) m_bPhase(%u) header(%u) who(%s)", m_lpOutputBuffer->mem_size, m_lpOutputBuffer->write_point_pos, iSize, m_bPhase, header, name.c_str());
					m_bPhase = PHASE_CLOSE;
				}
			}
			else
...
void DESC::SetPhase(uint8_t _phase)
{
	m_bPhase = _phase;
...
const BYTE* GetKey_20050304Myevan()
{
	...
	if (!bGenerated)
	{
		...
		for (uint32_t i = 0; i < BYTE(seed); ++i)
		{
			seed ^= 2148941891ul;
			seed += 3592385981ul;
...
{
	static constexpr uint32_t delay = 30; // sec
	return get_global_time() - m_chLastStatusReqTime > delay;
}

void DESC::SetChannelStatusRequested()
{
	m_chLastStatusReqTime = get_global_time();
}

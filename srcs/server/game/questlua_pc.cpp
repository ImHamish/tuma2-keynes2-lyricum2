...
namespace quest
{
	...
	int pc_change_sex(lua_State* L)
	{
		...
		LPITEM item = ch->GetWear(WEAR_COSTUME_BODY);
		if (item) {
			lua_pushnumber(L, 2);
			return 1;
		}
	...
	{
#if defined(ENABLE_NEW_GOLD_LIMIT)
		int64_t gold = static_cast<int64_t>(lua_tonumber(L, -1));
#else
		signed long gold = static_cast<signed long>(lua_tonumber(L, -1));
#endif
...
#else
			sys_err("QUEST wrong ChangeGold %lu (now %lu)", gold, ch->GetGold());
#endif
		}
		else
		{
	...
	int pc_aggregate_monster2(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
...
teleport_area:
	...
	int pc_give_award_socket(lua_State* L)
	{
		...
		DBManager::instance().Query("INSERT INTO item_award (login, vnum, count, given_time, why, mall, socket0, socket1, socket2"
#if defined(USE_SOCKET_MAX_NUM)
			", socket3"
#endif
			")select '%s', %d, %d, now(), '%s', 1, %s, %s, %s"
#if defined(USE_SOCKET_MAX_NUM)
			", %s"
#endif
			" from DUAL where not exists (select login, why from item_award where login = '%s' and why  = '%s') ;",
			ch->GetDesc()->GetAccountTable().login,
			dwVnum,
			icount,
			lua_tostring(L,3),
	...
	int pc_get_killee_drop_pct(lua_State* L)
	{
		...
		if (NULL == pKillee || !ITEM_MANAGER::instance().GetDropPct(pKillee, pChar, iDeltaPercent, iRandRange, iRandRangeLimit, iDeltaLimitRange))
		{
		...
		if (pkNewItem)
		{
			...
			switch(m_state)
			{
				...
				case PCMI0_DROP:
				{
                    ...
                    if (ch->GetFilterStatus(FILTER_TYPE_AUTO) && ch->CanPickItem(pkNewItem))
                    {
                        ch->AutoGiveItem(pkNewItem, false
...
, true
	...
	int pc_open_biologist_change(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch) {
	...
	int pc_have_items_weared(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		for (uint8_t i = 0; i < WEAR_MAX_NUM; i++) {
			if (ch->GetWear(WEAR_BODY + i) != NULL) {
				isEquipped = true;
				break;
			}
		}

		lua_pushboolean(L, isEquipped);
		return 1;
	}
	...
	int pc_open_gem_shop(lua_State* L)
	{
		...
		if (!ch->CanWarp()) {
			ch->ChatPacketNew(CHAT_TYPE_INFO, 776, "");
			return 0;
		}
	...
	int pc_change_gaya(lua_State* L)
	{
		...
		if (gaya + ch->GetGem() < 0) {
			sys_err("QUEST wrong ChangeGaya %d (now %d)", gaya, ch->GetGem());
		} else {
#if defined(USE_BATTLEPASS)
			if (gaya > 0) {
				ch->UpdateExtBattlePassMissionProgress(GAYA_CRAFT_GAYA, gaya, 0);
			}
#endif
			ch->PointChange(POINT_GEM, gaya, true);
		}
	...
	int pc_is_blocked(lua_State* L)
	{
		...
		if (!pkChar) {
			sys_err("Invalid character pointer.");
			return 0;
		}
		...
		const LPCHARACTER pkTargetChar = CHARACTER_MANAGER::instance().FindPC(targetName);

		if (!pkTargetChar || !pkTargetChar->IsPC())
		{
			sys_err("Invalid target character pointer.");
			return 0;
		}

		lua_pushboolean(L, MessengerManager::instance().CheckMessengerList(pkChar->GetName(), pkTargetChar->GetName(), SYST_BLOCK));
		return 1;
	}
	...
	int pc_is_friend(lua_State* L)
	{
		...
		lua_pushboolean(L, MessengerManager::instance().CheckMessengerList(pkChar->GetName(), pkTargetChar->GetName(), SYST_FRIEND));
		return 1;
	}
#endif

#if defined(ENABLE_RENEWAL_OX)
	int pc_remove_good_affect(lua_State* L)
	{
		const LPCHARACTER pkChar = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int pc_get_skull(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
			return 0;
		}

		lua_pushnumber(L, pChar->GetSkull());
		return 1;
	}

	int pc_give_skull(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
		...
		if (amount <= 0) {
			sys_err("QUEST: skull amount less then zero");
			return 0;
		}

		pChar->PointChange(POINT_SKULL, amount, true);

		return 0;
	}

	int pc_change_skull(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
		...
		if (amount + pChar->GetSkull() < 0) {
			sys_err("QUEST wrong change skull %d (now %d)", amount, pChar->GetSkull());
		} else {
			pChar->PointChange(POINT_SKULL, amount, true);
		}
	...
	int pc_get_wb_points(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
		...
		lua_pushnumber(L, pChar->GetWbPoints());
		return 1;
	}

	int pc_give_wb_points(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
		...
		pChar->PointChange(POINT_WB_POINTS, amount, true);

		return 0;
	}

	int pc_change_wb_points(lua_State* L)
	{
		const LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pChar) {
			sys_err("QUEST: no character pointer.");
		...
		if (amount + pChar->GetSkull() < 0) {
			sys_err("QUEST wrong change wb points %d (now %d)", amount, pChar->GetSkull());
		} else {
			pChar->PointChange(POINT_WB_POINTS, amount, true);
		}
	...
	int pc_get_okay_global_rank(lua_State* L)
	{
		if (!lua_isstring(L, 1) || !lua_isstring(L, 2))
		{
			sys_err("QUEST: not enough arguments.");
			return 0;
		}
		...
		char szBuf[4096 + 1];
		ch->GetGlobalRank(lua_tostring(L, 1), lua_tostring(L, 2), szBuf, sizeof(szBuf));
	...
	int pc_get_okay_rund_rank(lua_State* L)
	{
		...
		char szBuf[4096 + 1];
		ch->GetRundRank(lua_tostring(L, 1), lua_tostring(L, 2), szBuf, sizeof(szBuf));
...
#ifdef ENABLE_VOTE4BONUS
    int32_t pc_choose_votebonus(lua_State* L)
    {
        if (!lua_isnumber(L, 1))
        {
            sys_err("Invalid bonus arg.");
            return 0;
        }
        ...
        if (!pDesc)
        {
            sys_err("This char pointer have no desc.");
            return 0;
        }

        const auto id = pDesc->GetAccountTable().id;
        if (!id)
        {
            lua_pushnumber(L, 1);
            return 1;
        ...
        if (pChar->GetPremiumRemainSeconds(PREMIUM_BONUS1) > 0
            || pChar->GetPremiumRemainSeconds(PREMIUM_BONUS2) > 0
            || pChar->GetPremiumRemainSeconds(PREMIUM_BONUS3) > 0)
        {
            lua_pushnumber(L, 0);
            return 1;
        }
        ...
        switch (index)
        {
            case 1:
            {
                premiumType = PREMIUM_BONUS1;
                std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE `account`.`account` SET `bonus1_expire` = NOW()+INTERVAL 1 DAY WHERE `id` = %u;", id));
                break;
            }
            case 2:
            {
                premiumType = PREMIUM_BONUS2;
                std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE `account`.`account` SET `bonus2_expire` = NOW()+INTERVAL 1 DAY WHERE `id` = %u;", id));
                break;
            }
            case 3:
            {
                premiumType = PREMIUM_BONUS3;
                std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE `account`.`account` SET `bonus3_expire` = NOW()+INTERVAL 1 DAY WHERE `id` = %u;", id));
                break;
            }
            default:
            {
                premiumType = PREMIUM_MAX_NUM;
                break;
            }
        }
        ...
        DBManager::instance().SetPremiumByType(pDesc, premiumType, get_global_time() + 86400);
...
#ifdef USE_AUTO_HUNT
    int32_t pc_give_premium_hunt(lua_State* L)
    {
        if (!lua_isnumber(L, 1))
        {
            sys_err("Invalid days arg.");
            lua_pushnumber(L, 0);
            return 1;
        }

        int32_t days = (int32_t)lua_tonumber(L, 1);
        if (days <= 0 || days > 30)
        {
            sys_err("invalid days amount.");
            lua_pushnumber(L, 0);
            return 1;
        }
        ...
        if (!id)
        {
            sys_err("invalid account id.");
            lua_pushnumber(L, 0);
            return 1;
        }

        auto left = pChar->GetPremiumRemainSeconds(PREMIUM_AUTO_USE);
        if (left < 0)
        {
            left = 0;
        }

        const auto total = (86400 * days) + left;

        std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE `account`.`account` SET `auto_use_expire` = NOW()+INTERVAL %d SECOND WHERE `id`=%u;", total, id));

        DBManager::instance().SetPremiumByType(pDesc, PREMIUM_AUTO_USE, get_global_time() + total);
	...
	int pc_is_premium(lua_State* L)
	{
		LPCHARACTER pkChar = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!pkChar)
			return 0;
		
		lua_pushboolean(L, pkChar->IsPremiumPlayer() == 1);
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
...
#endif
#if defined(ENABLE_WORLDBOSS)
			{"get_wb_points", pc_get_wb_points},
			{"give_wb_points", pc_give_wb_points},
			{"change_wb_points", pc_change_wb_points},
#endif
#if defined(ENABLE_HALLOWEEN_EVENT_2022)
			{"get_skull", pc_get_skull},
			{"give_skull", pc_give_skull},
			{"change_skull", pc_change_skull},
#endif
#if defined(ENABLE_CHANGELOOK)
			{"open_changelook", pc_open_changelook},
#endif
#if defined(ENABLE_RENEWAL_OX)
			{"remove_good_affect", pc_remove_good_affect},
#endif
#if defined(ENABLE_MESSENGER_BLOCK)
...
#endif
			{"have_items_weared", pc_have_items_weared},
			...
			{ "aggregate_monster2",	pc_aggregate_monster2	},
...
#endif
#ifdef ENABLE_VOTE4BONUS
            { "choose_votebonus", pc_choose_votebonus },
#endif
#ifdef USE_AUTO_HUNT
            { "give_premium_hunt", pc_give_premium_hunt },
#endif
			{NULL, NULL}
		};

...
CSafebox::CSafebox(LPCHARACTER pkChrOwner, int32_t iSize, DWORD dwGold) : m_pkChrOwner(pkChrOwner), m_iSize(iSize), m_lGold(dwGold) {
...
int32_t CSafebox::GetEmptySafebox(BYTE size)
{
	for (int32_t i = 0; m_pkGrid && i < m_pkGrid->GetSize(); i++) {
		if (IsEmpty(i, size)) {
			return i;
		}
	}
...
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, WORD count)
{
	...
	if (m_pkChrOwner->IsLastMoveItemTime()) {
		m_pkChrOwner->ChatPacketNew(CHAT_TYPE_INFO, 1532, "");
		return false;
	}

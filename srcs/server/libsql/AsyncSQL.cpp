...
#endif
{
	CAsyncSQL* pSQL = static_cast<CAsyncSQL*>(arg);
...
bool CAsyncSQL::QueryLocaleSet()
{
	...
	{
		sys_err("cannot set locale %s by 'mysql_set_character_set', errno %u %s", m_stLocale.c_str(),
		        mysql_errno(&m_hDB), mysql_error(&m_hDB));
		return false;
	}
...
bool CAsyncSQL::Setup(const char* c_pszHost, const char* c_pszUser, const char* c_pszPassword, const char* c_pszDB,
                      const char* c_pszLocale, bool bNoThread, int32_t iPort)
{
	m_stHost = c_pszHost;
	m_stUser = c_pszUser;
	...
	if (!bNoThread)
	{
		...
		*/
		...
		m_hThread = reinterpret_cast<HANDLE>(::_beginthreadex(nullptr, 0, AsyncSQLThread, this, 0, nullptr));

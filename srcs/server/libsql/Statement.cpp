...
{
	sys_err("%s: [%d] %s", c_pszMsg, mysql_stmt_errno(m_pkStmt), mysql_stmt_error(m_pkStmt));
}
...
bool CStmt::Prepare(CAsyncSQL* sql, const char* c_pszQuery)
{
	...
	if (iParamCount)
	{
		...
		m_puiParamLen = static_cast<uint32_t*>(calloc(iParamCount, sizeof(uint32_t)));
	}
...
bool CStmt::BindParam(enum_field_types type, void* p, int32_t iMaxLen)
{
	...
	bind->length = reinterpret_cast<u_long*>(m_puiParamLen) + m_uiParamCount;

...
#endif
#if defined(ENABLE_NEW_FISH_EVENT)
		, pkTab->fishEventCount
#endif
		);

	static char text[8192 + 1];

	CDBManager::instance().EscapeString(text, pkTab->country_flag, sizeof(pkTab->country_flag));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "`country_flag` = '%s', ", text);
...
#if defined(ENABLE_NEW_FISH_EVENT)
	CDBManager::instance().EscapeString(text, pkTab->fishEventSlot, sizeof(pkTab->fishEventSlot));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ", fish_event_slot = '%s'", text);
#endif

#if defined(ENABLE_HIDE_COSTUME_SYSTEM)
	CDBManager::instance().EscapeString(text, pkTab->partStatus, sizeof(pkTab->partStatus));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, ", part_stats = '%s'", text);
#endif
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
...
#else
		sys_log(0, "[PLAYER_LOAD] ID %s pid %d gold %lu ", pTab->name, pTab->id, pTab->gold);
#endif
		//--------------------------------------------
		// 아이템 & AFFECT & QUEST 로딩 :
		//--------------------------------------------
...
#endif
#if defined(ENABLE_NEW_FISH_EVENT)
				",fish_event_count, fish_event_slot"
#endif
#if defined(ENABLE_HIDE_COSTUME_SYSTEM)
				",part_stats"
#endif
	...
	snprintf(queryStrBP, sizeof(queryStrBP),
		"SELECT player_id, mission_index, mission_type+0, battle_pass_id, "
		"extra_info, completed FROM battlepass_missions WHERE player_id = %d", packet->player_id);
...
#if defined(ENABLE_WORLDBOSS)
	str_to_number(pkTab->wbpoints, row[col++]);
#endif
#if defined(ENABLE_HALLOWEEN_EVENT_2022)
	str_to_number(pkTab->skull, row[col++]);
#endif
#if defined(ENABLE_GAYA_RENEWAL)
...
#endif
#ifdef __ENABLE_PREMIUM_PLAYERS__
	str_to_number(pkTab->premium, row[col++]);
	str_to_number(pkTab->premium_time, row[col++]);
#endif
...
#endif
#if defined(ENABLE_PENDANT_RENEWAL)
	str_to_number(pkTab->bPendant, row[col++]);
#endif
#if defined(ENABLE_GAYA_RENEWAL)
...
#if defined(ENABLE_NEW_FISH_EVENT)
	str_to_number(pkTab->fishEventCount, row[col++]);

	if (row[col]) {
		memcpy(pkTab->fishEventSlot, row[col], sizeof(pkTab->fishEventSlot));
	} else {
		memset(pkTab->fishEventSlot, 0, sizeof(pkTab->fishEventSlot));
	}
...
#if defined(ENABLE_HIDE_COSTUME_SYSTEM)
	if (row[col]) {
		memcpy(pkTab->partStatus, row[col], sizeof(pkTab->partStatus));
	} else {
		memset(pkTab->partStatus, 0, sizeof(pkTab->partStatus));
	}

	col++;
#endif

	strlcpy(pkTab->country_flag, row[col++], sizeof(pkTab->country_flag));
...
void CClientManager::QUERY_SAVE_EXT_BATTLE_PASS(CPeer* peer, DWORD dwHandle, TPlayerExtBattlePassMission* battlePass)
{
	if (g_test_server)
	{
		sys_log(0, "QUERY_SAVE_EXT_BATTLE_PASS: %u", battlePass->dwPlayerId);
	}

	char szQuery[QUERY_MAX_LEN];
	snprintf(szQuery, sizeof(szQuery),
		"REPLACE INTO battlepass_missions (player_id, mission_index, mission_type, battle_pass_id, extra_info, completed) VALUES (%u, %u, %u, %u, %u, %u)",
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
...
#if defined(ENABLE_WORLDBOSS)
			, packet->player_table.wbpoints
#endif
#if defined(ENABLE_HALLOWEEN_EVENT_2022)
			, packet->player_table.skull
#endif
#if defined(ENABLE_GAYA_RENEWAL)
...
#else
	sys_log(0, "PlayerCreate accountid %d name %s level %d gold %lu, st %d ht %d job %d",
#endif
			packet->account_id,
...
#if defined(ENABLE_NEW_FISH_EVENT)
	CDBManager::instance().EscapeString(text, packet->player_table.fishEventSlot, sizeof(packet->player_table.fishEventSlot));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif

#if defined(ENABLE_HIDE_COSTUME_SYSTEM)
	CDBManager::instance().EscapeString(text, packet->player_table.partStatus, sizeof(packet->player_table.partStatus));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s'", text);
#endif
	...
	CDBManager::instance().EscapeString(text, packet->player_table.country_flag, sizeof(packet->player_table.country_flag));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", '%s')", text);
	...
	strlcpy(pack.player.country_flag, packet->player_table.country_flag, sizeof(pack.player.country_flag));

...
bool CClientManager::InitializeItemExtraProtoTable()
{
	...
	std::unique_ptr<SQLMsg> msg(CDBManager::instance().DirectQuery("SELECT * FROM item_extra_proto ORDER by vnum ASC"));
	...
	for (auto it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				"%d, %d, %d, \"%s\", \"%s\", %llu, %llu, %d, %d, " //11
#else
				"%u, %u, %u, \"%s\", \"%s\", %lu, %lu, %u, %u, " //11
#endif

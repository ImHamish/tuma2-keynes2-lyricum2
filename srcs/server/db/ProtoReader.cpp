...
int get_Item_SubType_Value(unsigned int type_value, string inputString)
{
	...
	static string arSub38[] = { "RED_SOUL", "BLUE_SOUL" };
...
{
	string arLimitType[] = {"LIMIT_NONE", "LEVEL", "STR", "DEX", "INT", "CON", "UNUSED", "REAL_TIME", "REAL_TIME_FIRST_USE", "TIMER_BASED_ON_WEAR"};
...
int get_Item_ApplyType_Value(string inputString)
{
	string arApplyType[] = {"APPLY_NONE", "APPLY_MAX_HP", "APPLY_MAX_SP", "APPLY_CON", "APPLY_INT", "APPLY_STR", "APPLY_DEX", "APPLY_ATT_SPEED",
			...
			"APPLY_SKILL_DEFEND_BONUS", "APPLY_NORMAL_HIT_DEFEND_BONUS", "APPLY_PC_UNUSED01", "APPLY_PC_UNUSED02",
...
bool Set_Proto_Mob_Table(TMobTable *mobTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
	...
	if (it != nameMap.end()) {
		...
		strlcpy(mobTable->szLocaleName[0], localeName, sizeof (mobTable->szLocaleName[0]));
	} else {
		strlcpy(mobTable->szLocaleName[0], mobTable->szName, sizeof (mobTable->szLocaleName[0]));
	}
#else
	if (it != nameMap.end()) {
		const char * localeName = it->second;
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
	...
	if (it != nameMap.end()) {
		const char * localeName = it->second;
		strlcpy(itemTable->szLocaleName[0], localeName, sizeof (itemTable->szLocaleName[0]));
	} else {
		strlcpy(itemTable->szLocaleName[0], itemTable->szName, sizeof (itemTable->szLocaleName[0]));
	}
#else
	if (it != nameMap.end()) {
		const char * localeName = it->second;
	...
	for (i = 0; i < ITEM_VALUES_MAX_NUM; ++i)
		itemTable->alValues[i] = dataArray[24 + j + i];

	//column for 'Specular'
	itemTable->bGainSocketPct = dataArray[31 + j];
	itemTable->sAddonType = dataArray[32 + j];

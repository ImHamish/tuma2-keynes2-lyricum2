...
#endif
					strlcpy(pkTab->players[j].country_flag, pt->country_flag, sizeof(pkTab->players[j].country_flag));
				}
				else
				{
					if (!row[col++])
...
#if defined(ENABLE_PENDANT_RENEWAL)
					col++; // pendant not used ?
#endif
					if (!row[col++])
						strlcpy(pkTab->players[j].country_flag, "ro", sizeof(pkTab->players[j].country_flag));
					else
						strlcpy(pkTab->players[j].country_flag, row[col - 1], sizeof(pkTab->players[j].country_flag));
				}
...
void CClientManager::QUERY_CHANGE_COUNTRY_FLAG(CPeer* peer, DWORD dwHandle, const TPacketGDChangeCountryFlag* p)
{
	if (!*p->login)
		return;

	CLoginData* pLoginData = GetLoginDataByLogin(p->login);
	if (!pLoginData)
		return;

	char szCountryFlag[3 * 2 + 1];
	CDBManager::instance().EscapeString(szCountryFlag, p->country_flag, strlen(p->country_flag));

	bool result = false;

	auto& account = pLoginData->GetAccountRef();
	for (auto& player : account.players)
	{
		if (player.dwID == p->pid)
		{
			sys_log(0, "pid %u change country flag from %s to %s", player.dwID, player.country_flag, szCountryFlag);

			strlcpy(player.country_flag, szCountryFlag, sizeof(player.country_flag));

			CPlayerTableCache* pc = GetPlayerCache(player.dwID);
			if (TPlayerTable* pt = pc ? pc->Get() : nullptr)
			{
				strlcpy(pt->country_flag, szCountryFlag, sizeof(pt->country_flag));
			}
			else
			{
				char szQuery[QUERY_MAX_LEN] = {};
				snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET `country_flag`='%s' WHERE `id`=%u;", GetTablePostfix(), szCountryFlag, p->pid);
	...
	if (!result) // if player is already created
	{
		char szQuery[QUERY_MAX_LEN] = {};
		snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET `country_flag`='%s' WHERE `id`=%u;", GetTablePostfix(), szCountryFlag, p->pid);
		CDBManager::instance().AsyncQuery(szQuery);
	...
	TPacketDGChangeCountryFlag pdg;
	peer->EncodeHeader(HEADER_DG_CHANGE_COUNTRY_FLAG, dwHandle, sizeof(pdg));
	pdg.pid = p->pid;
	strlcpy(pdg.country_flag, szCountryFlag, sizeof(szCountryFlag));
	peer->Encode(&pdg, sizeof(pdg));
}

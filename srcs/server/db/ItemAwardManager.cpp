...
void ItemAwardManager::Load(SQLMsg * pMsg)
{
	...
	for (uint32_t i = 0; i < pMsg->Get()->uiNumRows; ++i)
	{
		...
		str_to_number(kData->alSockets[0], row[col++]);
		str_to_number(kData->alSockets[1], row[col++]);
		str_to_number(kData->alSockets[2], row[col++]);
#if defined(USE_SOCKET_MAX_NUM)
		str_to_number(kData->alSockets[3], row[col++]);
#endif
		...
		sys_log(0, "ITEM_AWARD: load id %lu login %s vnum %lu count %u socket %lu", kData->dwID, kData->szLogin, kData->dwVnum, kData->dwCount, kData->alSockets[0]);
...
void ItemAwardManager::Taken(DWORD dwAwardID, DWORD dwItemID)
{
	...
	{
		sys_err("ITEM_AWARD: Taken ID not exist dwAwardID(%u) dwItemID(%u)", dwAwardID, dwItemID);
		return;
	}
...
void ItemAwardManager::CheckItemSkillBook(TItemAward & rkItemAward, const std::vector<TSkillTable> vec_skillTable)
{
	...
	uint32_t SkillVnum = rkItemAward.alSockets[0];
	bool bHasSkill = false;
	for (size_t i = 0; i < vec_skillTable.size(); ++i)
	{
		if (vec_skillTable[i].dwVnum == SkillVnum)
		{
			bHasSkill = true;
			break;
		}
	...
	if (!bHasSkill)
	{
		do
		{
			...
			SkillVnum = dwSkillVnum;
			break;
		}
		while (true);

		rkItemAward.alSockets[0] = SkillVnum;
	}
}
...
void ItemAwardManager::CheckItemBlend(TItemAward & rkItemAward, const TItemTable & rkItemTable)
{
	...
	const uint32_t bApplyType = rkItemAward.alSockets[0];
	const uint32_t bApplyValue = rkItemAward.alSockets[1];
	const uint32_t dwApplyDuration = rkItemAward.alSockets[2];
...
void ItemAwardManager::CheckItemSocket(TItemAward & rkItemAward, const TItemTable & rkItemTable)
{
	...
	if (maxSockets >= 1)
		rkItemAward.alSockets[0] = 1;
	if (maxSockets >= 2)
		rkItemAward.alSockets[1] = 1;
	if (maxSockets >= 3)
		rkItemAward.alSockets[2] = 1;
#if defined(USE_SOCKET_MAX_NUM)
	if (maxSockets >= 4)
		rkItemAward.alSockets[3] = 1;
#endif
}
